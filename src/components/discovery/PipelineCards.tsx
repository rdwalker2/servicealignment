  ...
  return (
    ...
    >
      ...
      <div className="pl-3.5 pr-3 py-2">
        ...
        {/* Row 1: Name + Deal Value / Score */}
        <div className="flex items-center justify-between gap-2 mb-1">
          <h4 className="text-[12px] font-semibold text-stone-800 truncate flex-1 leading-tight">
            {session.company_name}
          </h4>
          <div className="flex items-center gap-2 shrink-0">
            {session.roof_health_score !== undefined && (
              <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${
                session.roof_health_score < 50 ? 'bg-red-100 text-red-700' : 
                session.roof_health_score < 80 ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'
              }`}>
                Score: {session.roof_health_score}
              </span>
            )}
