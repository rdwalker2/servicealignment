...
import {
  ...
  Check, Loader2, Copy, AlertTriangle,
} from 'lucide-react';
...
interface ContactDrawerProps {
  ...
  isPushed?: boolean;
  isRyanMode: boolean;
  /** Internal rep ID for call attribution (e.g. 'rep-jl') */
  callerRepId?: string;
  /** Whether this contact is marked Do Not Contact in Salesloft */
...
function ContactDrawer({ contact, account, onClose, onAddToCadence, onAddToAimfox, isPushed, isRyanMode, callerRepId, doNotContact }: ContactDrawerProps) {
      ...
      >
        ...
        <div className="shrink-0 border-b border-gray-100 px-5 pt-4 pb-3">
          ...
          <div className="flex gap-2 mb-1">
            ...
            {onAddToAimfox && (
              <button
                onClick={() => onAddToAimfox(contact.email, contact)}
                className="flex items-center justify-center gap-1.5 px-4 py-[8px] rounded-lg text-[11px] font-semibold transition-all bg-blue-600 text-white hover:bg-blue-700 shadow-sm"
              >
                Aimfox
              </button>
            )}
