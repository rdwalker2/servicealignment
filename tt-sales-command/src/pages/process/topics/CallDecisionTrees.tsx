  ...
  },
  {
    ...
    paths: [
      ...
      },
      {
        ...
        microYes: `(Yes — they always have workarounds) → "Aptitude Research found 82% of companies report significant gaps in their ATS and only 3% actually use it fully. Do you ever check what else is out there — even just to benchmark?"`,
        close: UNIVERSAL_CLOSE,
        closeType: 'meeting',
      },
      {
...
function StepLabel({ label, color }: { label: string; color: string }) {
  return (
    <p className={`text-[9px] font-black uppercase tracking-[0.15em] mb-1 ${color}`}>{label}</p>
  );
}

// ─── Close Type Badge ─────────────────────────────────────────────

function CloseBadge({ type }: { type: TreePath['closeType'] }) {
  const config = {
    meeting: { label: '→ SET THE MEETING', bg: 'bg-emerald-50', text: 'text-emerald-700', border: 'border-emerald-200' },
    wave3: { label: '→ ENTER WAVE 3', bg: 'bg-rose-50', text: 'text-rose-700', border: 'border-rose-200' },
    callback: { label: '→ LOCK CALLBACK', bg: 'bg-amber-50', text: 'text-amber-700', border: 'border-amber-200' },
  }[type];

  return (
    <span className={`inline-block text-[9px] font-black uppercase tracking-[0.15em] px-2 py-1 rounded ${config.bg} ${config.text} border ${config.border}`}>
      {config.label}
    </span>
  );
}
...
function PathCard({ path, index }: { path: TreePath; index: number }) {
  const borderColors = [
    'border-l-rose-400',
    'border-l-violet-400',
    'border-l-blue-400',
  ...
  return (
    <div className={`rounded-lg border border-stone-200 border-l-[3px] ${borderColors[index]} overflow-hidden`}>
      {/* Objection — what they say */}
      <div className="px-4 py-3 bg-stone-50 border-b border-stone-100">
        <StepLabel label="❶ They Say" color="text-stone-400" />
        <p className="text-[13px] font-semibold text-stone-800">{path.objection}</p>
      </div>

      <div className="px-4 py-3 space-y-0">
        {/* Validate */}
        <div>
          <StepLabel label="❷ Validate" color="text-emerald-600" />
          <p className="text-[12.5px] text-stone-700">{path.validate}</p>
        </div>
        ...
        {/* Reframe as problem */}
        <div>
          <StepLabel label="❸ Reframe" color="text-amber-600" />
          <p className="text-[12.5px] text-stone-700">{path.reframe}</p>
        </div>

        <Arrow />

        {/* Micro-yes / rhetorical */}
        <div>
          <StepLabel label="❹ Micro-Yes" color="text-blue-600" />
          <p className="text-[12.5px] text-stone-700">{path.microYes}</p>
        </div>
        ...
        <div className="p-3 bg-stone-50 rounded-lg border border-stone-100">
          <CloseBadge type={path.closeType} />
          <p className="text-[12.5px] text-stone-700 mt-2">{path.close}</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="p-10 max-w-5xl mx-auto h-full overflow-y-auto pb-32">
      ...
      <div className="space-y-6">
        {/* Opener */}
        <div className="border-l-[3px] border-[#FF2A7F] pl-6 py-5">
          <div className="flex items-center justify-between mb-3">
            <p className="text-[10px] font-semibold uppercase tracking-widest text-stone-400">
              Your Opener — {tree.persona}
            </p>
            <span className="text-[10px] px-2 py-1 rounded-full bg-stone-100 text-stone-400 font-semibold">
              {tree.subtitle}
            </span>
          </div>
          <p className="text-[15px] leading-relaxed font-medium text-stone-800">{tree.opener}</p>
          <p className="text-[11px] text-stone-400 mt-3 italic">{tree.openerNote}</p>
        </div>
        ...
        <div className="flex justify-center">
          <div className="flex flex-col items-center">
            ...
            <div className="text-[10px] font-black text-stone-400 uppercase tracking-widest bg-stone-100 px-3 py-1 rounded-full">
              They respond with one of 4 things…
            </div>
            <div className="w-0.5 h-4 bg-stone-300" />
          </div>
        </div>
      ...
      <DocSection title="The Strategy Behind Every Step">
        <p className="text-[13px] text-stone-500 mb-4">These trees aren't random — every step uses a specific technique from the training framework, adapted for ATS sales.</p>
