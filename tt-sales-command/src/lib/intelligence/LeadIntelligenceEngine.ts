 ...
 * to extract their current tech stack (ATS), hiring pains (via Glassdoor/Indeed reviews),
  ...
  public static async scanDomain(domain: string): Promise<IntelligenceResult> {
    ...
    if (normalizedDomain.includes('stripe') || normalizedDomain.includes('uber') || normalizedDomain.includes('airbnb')) {
      return {
        ...
        displacementAngle: 'Greenhouse is slowing down their engineering hires. Teamtailor can cut time-to-hire by 30% while offering a candidate experience that engineers actually respect.',
      };
    }
    ...
    if (normalizedDomain.includes('walmart') || normalizedDomain.includes('starbucks') || normalizedDomain.includes('target')) {
      return {
        ...
        displacementAngle: 'Workday is a massive ERP, but terrible for high-volume retail hiring. Teamtailor\'s mobile-first apply process will capture the 40% of retail workers currently abandoning the Workday application.',
      };
    }
    ...
    if (normalizedDomain.includes('vercel') || normalizedDomain.includes('linear') || normalizedDomain.includes('notion')) {
      return {
        ...
        displacementAngle: 'They have an incredible consumer brand, but their Lever careers page looks just like everyone else\'s. Teamtailor lets them build a stunning, bespoke employer brand to attract passive 10x engineers.',
      };
    }
    ...
    return {
      ...
      signals: [
        'Generic applicant tracking system detected.',
        'High candidate drop-off rate on standard application forms.',
        'No active talent pool nurturing campaigns found.',
      ],
      displacementAngle: 'Their current process is highly transactional. Teamtailor will help them shift to a proactive talent pooling model with a superior candidate experience.',
    };
  }
}
