...
export interface DbAccount {
  ...
  employee_size_range?: string;  // SFDC picklist: "26-100", "101-250", etc.
  current_ats?: string;
  company_type?: string;
  size_segment?: string;
  icp_fit?: string;
...
export async function fetchAllAccounts(): Promise<DbAccount[]> {
  // Supabase REST defaults to 1000 rows max — paginate to get all
  const all: DbAccount[] = [];
  const pageSize = 1000;
  let from = 0;
  ...
  while (true) {
    ...
    all.push(...(data as DbAccount[]));

    if (data.length < pageSize) break;
    from += pageSize;
  }

  console.log(`[DB] Fetched ${all.length} total accounts`);
  return all;
}

export interface DbContact {
  email: string;
  name: string;                     // Single 'name' column in the actual table
  title?: string;
  phone?: string;
  linkedin_url?: string;
  domain?: string;
...
export async function fetchAllContacts(): Promise<DbContact[]> {
  ...
  while (true) {
    ...
    if (!data || data.length === 0) break;
    all.push(...(data as DbContact[]));

    if (data.length < pageSize) break; // last page
    from += pageSize;
  }

  console.log(`[DB] Fetched ${all.length} total contacts`);
  return all;
}
