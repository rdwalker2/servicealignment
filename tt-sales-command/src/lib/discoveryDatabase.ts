...
export interface ROIInputs {
  ...
  timeToHire: number;
  // Assumptions
  legacyAtsSeatCost?: number;
  avgAgencyFee?: number;
  expectedOrganicShift?: number;
  expectedTimeSavings?: number;
  lostRevenuePerEmptyDay?: number;
  // New SMB Modules
  annualJobBoardSpend?: number;
  expectedJobBoardSavings?: number;
  hrHourlyRate?: number;
  hmHourlyRate?: number;
  hrHoursSavedPerHire?: number;
  hmHoursSavedPerHire?: number;
}
...
export interface DiscoverySession {
  ...
  company_domain: string | null;           // Domain from Prospecting view (links to accounts table)
  // ── Salesforce IDs (canonical dedup keys) ──
  sfdc_opportunity_id?: string;            // Salesforce Opportunity ID — primary dedup key
  sfdc_account_id?: string;                // Salesforce Account ID
  sfdc_contact_id?: string;                // Salesforce primary Contact ID
  persona: string | null;              // Selected persona tab (dir-eb, vp-ta, chro, hiring-manager)
  current_ats: string | null;          // Competitor ATS (greenhouse, ashby, bamboohr, lever, workday, icims, smartrecruiters, other)
  industry: string | null;             // Prospect's industry vertical
  company_size: string | null;         // Employee count bracket (1-50, 51-200, 201-500, etc.)
  use_case: string | null;             // Primary buying motivation (replacing-ats, first-ats, adding-brand, consolidating, scaling)
  ...
  call_sheet_target_date: string;                         // "Date they want it solved by" field

  // ── CS Handoff Fields ──
  // Technical Environment
  handoff_current_hris: string;            // Workday, BambooHR, SAP SF, etc.
  handoff_calendar_system: string;         // Google Calendar, Outlook, O365
  handoff_sso_provider: string;            // Okta, Azure AD, OneLogin, etc.
  handoff_sso_required: boolean;
  handoff_infosec_review_required: boolean;
  // Career Site & Brand
  handoff_current_career_site_url: string;
  handoff_custom_domain: string;           // e.g., careers.acme.com
  handoff_brand_maturity: 'none' | 'basic' | 'advanced' | '';
  // Org Structure
  handoff_department_count: number;
  handoff_location_count: number;
  handoff_active_hiring_managers: number;
  handoff_active_recruiters: number;
  // Data Migration
  handoff_active_jobs_count: number;
  handoff_candidate_count_estimate: number;
  handoff_data_migration_notes: string;
  // Integrations
  handoff_job_boards: string[];            // Indeed, LinkedIn, etc.
  handoff_hris_integration: string;        // Which HRIS to integrate
  handoff_other_integrations: string[];    // Slack, assessment tools, etc.
  // Go-Live
  handoff_target_go_live_date: string;     // ISO date string
  handoff_launch_type: 'phased' | 'big-bang' | '';
  handoff_go_live_notes: string;
  // Training
  handoff_users_to_train: number;
  handoff_training_format: 'live' | 'webinar' | 'self-serve' | '';
  handoff_training_notes: string;
  // Enhanced Contacts
  handoff_primary_contact_email: string;
  handoff_it_contact_email: string;
  handoff_preferred_comm_channel: 'email' | 'slack' | 'teams' | '';
  // Success KPIs (structured)
  handoff_target_time_to_hire: number;     // Target days
  handoff_target_cost_per_hire: number;    // Target $
  handoff_target_adoption_rate: number;    // Target % hiring manager adoption
  handoff_success_review_date: string;     // 90-day review date
  // Prospect Pre-Work
  handoff_prework_items: string[];         // Items prospect committed to preparing
  handoff_prework_notes: string;
  // Compliance
  handoff_gdpr_applicable: boolean;
  handoff_data_retention_months: number;
  handoff_dpa_required: boolean;
  handoff_compliance_notes: string;

  // ── Deal Intelligence Fields (from transcript analysis) ──
  contract_end_date: string;              // When does their current ATS contract end?
...
function getAllSessions(): DiscoverySession[] {
  try {
    ...
    const result = sessions.map(s => ({
      ...
      company_domain: (s as any).company_domain ?? null,
      ...
      sfdc_opportunity_id: (s as any).sfdc_opportunity_id ?? undefined,
      sfdc_account_id: (s as any).sfdc_account_id ?? undefined,
      sfdc_contact_id: (s as any).sfdc_contact_id ?? undefined,
    }));
    ...
    // Deduplicate by session ID first (exact dupes)
    const seenIds = new Set<string>();
    let deduped = result.filter(s => {
      if (seenIds.has(s.id)) return false;
      seenIds.add(s.id);
      return true;
    });
    // ── Shared scoring function for dedup winner selection ──
    const scoreSession = (sess: DiscoverySession) => {
      let sc = 0;
      if (sess.milestones && Object.keys(sess.milestones).length > 0) sc += 100;
      if (sess.deal_value && sess.deal_value > 0) sc += 50;
      if (sess.stakeholders && sess.stakeholders.length > 0) sc += 20;
      if (sess.next_steps_who || sess.next_steps_what || sess.next_steps_when) sc += 15;
      if (sess.most_recent_update) sc += 15;
      if (sess.champion_name) sc += 10;
      if (sess.pain_narrative) sc += 10;
      if (sess.compelling_event) sc += 10;
      if (sess.decision_criteria) sc += 10;
      if (sess.granola_notes && sess.granola_notes.length > 0) sc += 30;
      sc += new Date(sess.created_at || 0).getTime() / 1e12; // tiebreak: newer
      return sc;
    };

    // ── Priority 0: Deduplicate by Salesforce Opportunity ID (canonical) ──
    const bySfdcOpp = new Map<string, DiscoverySession>();
    for (const s of deduped) {
      const oppId = (s as any).sfdc_opportunity_id;
      if (!oppId) continue;
      const existing = bySfdcOpp.get(oppId);
      if (!existing) {
        bySfdcOpp.set(oppId, s);
      } else {
        // Merge loser fields into winner
        const winner = scoreSession(s) > scoreSession(existing) ? s : existing;
        const loser = winner === s ? existing : s;
        const mergeFields: (keyof DiscoverySession)[] = [
          'deal_value', 'next_steps_who', 'next_steps_what', 'next_steps_when',
          'most_recent_update', 'champion_name', 'pain_narrative', 'compelling_event',
          'decision_criteria', 'decision_process', 'paper_process', 'economic_buyer_access',
          'competitive_situation', 'success_metrics_text', 'forecast_category',
          'deal_stage', 'implementation_timeline', 'lead_source',
        ];
        for (const field of mergeFields) {
          if (!(winner as any)[field] && (loser as any)[field]) {
            (winner as any)[field] = (loser as any)[field];
          }
        }
        bySfdcOpp.set(oppId, winner);
      }
    }
    if (bySfdcOpp.size > 0) {
      const sfdcWinners = new Set(Array.from(bySfdcOpp.values()).map(s => s.id));
      deduped = deduped.filter(s => {
        const oppId = (s as any).sfdc_opportunity_id;
        if (!oppId) return true;
        return sfdcWinners.has(s.id);
      });
    }

    // ── Priority 1: Deduplicate by company_id ──

    const byCompany = new Map<string, DiscoverySession>();
    for (const s of deduped) {
      ...
      if (!existing) {
        ...
        if (scoreSession(s) > scoreSession(existing)) {
          byCompany.set(key, s);
        }
      }
    }
    // Build final list: sessions without company_id pass through, dupes are resolved
    const companyWinners = new Set(Array.from(byCompany.values()).map(s => s.id));
    const companiesWithDupes = new Set<string>();
    for (const s of deduped) {
      if (s.company_id && byCompany.has(s.company_id) && !companyWinners.has(s.id)) {
        companiesWithDupes.add(s.company_id);
      }
    }
    deduped = deduped.filter(s => {
      if (!s.company_id) return true; // no company_id = keep
      if (companyWinners.has(s.id)) return true; // winner = keep
      return false; // loser = drop
    });

    // ── Second pass: deduplicate by company_name + rep_id ──
    // This catches duplicates from different sources (Granola, manual, Supabase)
    // that have different company_id values but same company name
    const byNameRep = new Map<string, DiscoverySession>();
    for (const s of deduped) {
      const name = (s.company_name || '').trim().toLowerCase().replace(/[^a-z0-9]/g, '');
      if (!name) continue;
      const key = `${name}::${s.rep_id || 'unknown'}`;
      const existing = byNameRep.get(key);
      if (!existing) {
        byNameRep.set(key, s);
      } else {
        // Keep the one with more data, merge key fields from loser into winner
        const winner = scoreSession(s) > scoreSession(existing) ? s : existing;
        const loser = winner === s ? existing : s;
        ...
        // Merge granola_notes arrays
        if (loser.granola_notes && loser.granola_notes.length > 0) {
          const existingNoteIds = new Set((winner.granola_notes || []).map(n => n.id));
          const newNotes = loser.granola_notes.filter(n => !existingNoteIds.has(n.id));
          if (newNotes.length > 0) {
            winner.granola_notes = [...(winner.granola_notes || []), ...newNotes];
          }
        }
        byNameRep.set(key, winner);
        companiesWithDupes.add(name);
      }
    }
    const nameRepWinners = new Set(Array.from(byNameRep.values()).map(s => s.id));
    deduped = deduped.filter(s => {
      const name = (s.company_name || '').trim().toLowerCase().replace(/[^a-z0-9]/g, '');
      if (!name) return true;
      const key = `${name}::${s.rep_id || 'unknown'}`;
      if (!byNameRep.has(key)) return true;
      return nameRepWinners.has(s.id);
    });

    if (deduped.length < result.length) {
      console.warn(`[getAllSessions] Removed ${result.length - deduped.length} duplicate sessions (${companiesWithDupes.size} companies had dupes), saving clean data`);
...
export function computeCompetitorsEvaluating(session: DiscoverySession): string[] {
  ...
  // Filter out the prospect's own current ATS
...
/** Look up active pipeline sessions by company domain */
export function getActiveSessionByDomain(domain: string): DiscoverySession | null {
  const endStages = new Set(['closed_won', 'closed_lost', 'disqualified']);
  return getAllSessions().find(s => 
    s.company_domain === domain && !endStages.has(s.deal_stage)
  ) || null;
}

/** Build a map of domain → { stage, sessionId } for all active pipeline sessions */
export function getActivePipelineByDomain(): Map<string, { stage: string; sessionId: string }> {
  const endStages = new Set(['closed_won', 'closed_lost', 'disqualified']);
  const map = new Map<string, { stage: string; sessionId: string }>();
  for (const s of getAllSessions()) {
    if (s.company_domain && !endStages.has(s.deal_stage)) {
      map.set(s.company_domain, { stage: s.deal_stage, sessionId: s.id });
    }
  }
  return map;
}
...
export function updateSessionFromGranola(sessionId: string, updates: Partial<DiscoverySession>): void {
  ...
  const fields: Array<keyof DiscoverySession> = [
    ...
    'most_recent_update', 'most_recent_update_source', 'most_recent_update_at',
  ];
...
export function scoreMEDDPICC(session: DiscoverySession): MEDDPICCScore {
  ...
  const statuses = [metrics, identify_pain, champion, competition, decision_criteria];
  return {
    metrics,
    identify_pain,
    champion,
...
const OWNER_TO_REP: Record<string, string> = {
  'jack luther': 'jack-luther',
  'tyler hanson': 'tyler-hanson',
  'moe aqel': 'moe-aqel',
};

const SFDC_STAGE_MAP: Record<string, string> = {
  'qualifying': 'discovery',
  'investigating': 'diagnosis',
  'evaluating': 'demonstrate',
...
const SFDC_FORECAST_MAP: Record<string, string> = {
  'pipeline': 'pipeline',
  'best case': 'best_case',
  'commit': 'commit',
  'closed': 'closed',
...
/**
 * Import deals from a parsed SFDC export.
 * Matches by company_name + rep_id first, then creates new if not found.
 * Seeds sfdc_opportunity_id, sfdc_account_id, and all MEDDPICC fields.
 */
export function importFromSFDCExport(deals: SFDCDeal[]): { created: number; updated: number; skipped: number } {
  const sessions = getAllSessions();
  let created = 0, updated = 0, skipped = 0;

  for (const deal of deals) {
    const repId = OWNER_TO_REP[(deal.owner || '').toLowerCase().trim()] || null;
    if (!repId) { skipped++; continue; }

    const normalizedName = (deal.account || '').trim().toLowerCase().replace(/[^a-z0-9]/g, '');

    // Find existing session by sfdc_opportunity_id first, then by company_name + rep
    let session = sessions.find(s => (s as any).sfdc_opportunity_id === deal.opp_id);
    if (!session) {
      session = sessions.find(s => {
        const sName = (s.company_name || '').trim().toLowerCase().replace(/[^a-z0-9]/g, '');
        return sName === normalizedName && s.rep_id === repId;
      });
    }

    if (session) {
      // Update existing session with SFDC data
      (session as any).sfdc_opportunity_id = deal.opp_id;
      (session as any).sfdc_account_id = deal.account_id;
      session.opportunity_name = session.opportunity_name || deal.opp_name || undefined;
      session.deal_value = session.deal_value || deal.amount || 0;
      session.lead_source = session.lead_source || (deal.source?.toLowerCase().includes('outbound') ? 'outbound' : deal.source?.toLowerCase().includes('inbound') ? 'inbound' : session.lead_source);
      session.forecast_category = SFDC_FORECAST_MAP[(deal.forecast || '').toLowerCase()] || session.forecast_category || 'pipeline';
      session.deal_stage = SFDC_STAGE_MAP[(deal.stage || '').toLowerCase()] || session.deal_stage || 'discovery';
      // Close date
      if (deal.close_date && !session.implementation_timeline) {
        session.implementation_timeline = deal.close_date;
      }
      // MEDDPICC fields — overwrite from SFDC/JSON source of truth if provided
      if (deal.competition) session.competitive_situation = deal.competition;
      if (deal.metrics) session.success_metrics_text = deal.metrics;
      if (deal.pain) session.pain_narrative = deal.pain;
      if (deal.champion) session.champion_name = deal.champion;
      if (deal.decision_criteria) session.decision_criteria = deal.decision_criteria;
      if (deal.economic_buyer) session.economic_buyer_access = deal.economic_buyer;
      if (deal.decision_process) session.decision_process = deal.decision_process;
      if (deal.paper_process) session.paper_process = deal.paper_process;
      if (deal.compelling_event) session.compelling_event = deal.compelling_event;
      // Next Steps
      if (deal.next_steps) {
        session.next_action = deal.next_steps;
        const whoMatch = deal.next_steps.match(/^Who:\s*(.+)$/im);
        const whatMatch = deal.next_steps.match(/^What:\s*(.+(?:\n(?!Who:|When:).+)*)$/im);
        const whenMatch = deal.next_steps.match(/^When:\s*(.+)$/im);
        session.next_steps_who = whoMatch ? whoMatch[1].trim() : '';
        session.next_steps_what = whatMatch ? whatMatch[1].trim() : '';
        session.next_steps_when = whenMatch ? whenMatch[1].trim() : '';
      }
      // Most Recent Update
      if (deal.mru) {
        session.most_recent_update = deal.mru;
        session.most_recent_update_source = 'manual';
        session.most_recent_update_at = new Date().toISOString();
      }
      updated++;
    } else {
      // Create new session from SFDC data
      const newSession = createDiscoverySession(repId, deal.account, null);
      (newSession as any).sfdc_opportunity_id = deal.opp_id;
      (newSession as any).sfdc_account_id = deal.account_id;
      newSession.opportunity_name = deal.opp_name || undefined;
      newSession.deal_value = deal.amount || 0;
      newSession.lead_source = deal.source?.toLowerCase().includes('outbound') ? 'outbound' : deal.source?.toLowerCase().includes('inbound') ? 'inbound' : null;
      newSession.forecast_category = SFDC_FORECAST_MAP[(deal.forecast || '').toLowerCase()] || 'pipeline';
      newSession.deal_stage = SFDC_STAGE_MAP[(deal.stage || '').toLowerCase()] || 'discovery';
      if (deal.close_date) newSession.implementation_timeline = deal.close_date;
      newSession.competitive_situation = deal.competition || '';
      newSession.success_metrics_text = deal.metrics || '';
      newSession.pain_narrative = deal.pain || '';
      newSession.champion_name = deal.champion || '';
      newSession.decision_criteria = deal.decision_criteria || '';
      newSession.economic_buyer_access = deal.economic_buyer || '';
      newSession.decision_process = deal.decision_process || '';
      newSession.paper_process = deal.paper_process || '';
      newSession.compelling_event = deal.compelling_event || '';
      if (deal.next_steps) {
        newSession.next_action = deal.next_steps;
        const whoMatch = deal.next_steps.match(/^Who:\s*(.+)$/im);
        const whatMatch = deal.next_steps.match(/^What:\s*(.+)$/im);
        const whenMatch = deal.next_steps.match(/^When:\s*(.+)$/im);
        if (whoMatch) newSession.next_steps_who = whoMatch[1].trim();
        if (whatMatch) newSession.next_steps_what = whatMatch[1].trim();
        if (whenMatch) newSession.next_steps_when = whenMatch[1].trim();
      }
      if (deal.mru) {
        newSession.most_recent_update = deal.mru;
        newSession.most_recent_update_source = 'manual';
        newSession.most_recent_update_at = new Date().toISOString();
      }
      // Don't push — it was already added by createDiscoverySession
      // But we need to re-save after setting SFDC fields on the newSession
      {
        const latestSessions = getAllSessions();
        const idx = latestSessions.findIndex(s => s.id === newSession.id);
        if (idx !== -1) {
          latestSessions[idx] = newSession;
  ...
  // ── Re-read sessions to get ALL data (creates + updates) ──
  // createDiscoverySession saves to its own copy of localStorage,
  // and our update path modified the original `sessions` array.
  // Save updates first, then re-read to combine everything.
  saveSessions(sessions); // save any in-place updates from the update path

  // Now re-read to get the full combined list
  const allSessions = getAllSessions();

  // ── Cleanup: remove sessions not in SFDC export ──
  const sfdcOppIds = new Set(deals.map(d => d.opp_id));
  const beforeCount = allSessions.length;
  const cleanSessions = allSessions.filter(s => {
    const oppId = (s as any).sfdc_opportunity_id;
    // Keep if it has a valid SFDC Opp ID from this import
    if (oppId && sfdcOppIds.has(oppId)) return true;
    // Keep prospecting cards (pre-pipeline, no SFDC ID expected)
    if (s.deal_stage === 'prospecting') return true;
    // Remove everything else — it's not in Salesforce's open pipeline
    return false;
  });
  const removed = beforeCount - cleanSessions.length;
  if (removed > 0) {
    console.warn(`[SFDC Import] Cleaned up ${removed} orphan sessions not in SFDC pipeline`);
  }

  saveSessions(cleanSessions);
  console.log(`[SFDC Import] Final: ${cleanSessions.length} sessions. Created: ${created}, Updated: ${updated}, Skipped: ${skipped}, Removed: ${removed}`);
  return { created, updated, skipped };
}

// Expose globally for browser console import
if (typeof window !== 'undefined') {
  (window as any).__importSFDC = importFromSFDCExport;
}
