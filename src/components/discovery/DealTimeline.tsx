...
import {
  ...
  Mail, Plus, Phone, MailOpen, Reply, PhoneCall, Users, Activity
} from 'lucide-react';
...
export function DealTimeline({ session, onSessionUpdate }: {
  ...
  const [engagementStats, setEngagementStats] = useState<{ contacts: number, active: number, emails: number, calls: number, lastEmail: string | null } | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!actErr && actData) {
        setSlActivities(actData as SalesloftActivity[]);
      }

      // 2. Fetch aggregate contact engagement
      const { data: contacts, error: contactErr } = await supabase
        .from('contacts')
        .select('emails_sent, calls_total, in_active_cadence, last_email_at')
        .eq('domain', domain);
      ...
      if (!contactErr && contacts) {
        ...
        contacts.forEach(c => {
          ...
          calls += (c.calls_total || 0);
          if (c.in_active_cadence) active++;
          if (c.last_email_at) {
            if (!lastEmail || new Date(c.last_email_at) > new Date(lastEmail)) {
              lastEmail = c.last_email_at;
            }
          }
        });
  ...
  return (
    <div>
      ...
      {engagementStats && engagementStats.contacts > 0 && (
        <div className="mb-4 bg-stone-50 border border-stone-200/60 rounded-lg p-2.5 flex items-center justify-between text-[10px] text-stone-600">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1"><Users size={11} className="text-stone-400" /> <span className="font-medium">{engagementStats.contacts} contacts</span></div>
            {engagementStats.active > 0 && (
              <div className="flex items-center gap-1 text-violet-600"><Activity size={11} /> <span className="font-semibold">{engagementStats.active} in cadence</span></div>
            )}
          </div>
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1"><Mail size={11} className={engagementStats.emails > 0 ? "text-blue-500" : "text-stone-300"} /> <span>{engagementStats.emails} sent</span></div>
            <div className="flex items-center gap-1"><Phone size={11} className={engagementStats.calls > 0 ? "text-emerald-500" : "text-stone-300"} /> <span>{engagementStats.calls} calls</span></div>
          </div>
        </div>
      )}
                    ...
                    {entry.attendees && entry.attendees.length > 0 && (
                      <div className="flex items-center gap-1 mt-0.5 flex-wrap">
                        {entry.attendees.map(a => (
                          <a 
                            key={a} 
                            href={`https://servicealignment.lightning.force.com/lightning/search/workspace?q=${encodeURIComponent(a)}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="text-[9px] font-semibold bg-blue-50 text-blue-600 hover:bg-blue-100 hover:text-blue-700 transition-colors px-1.5 py-0.5 rounded inline-flex items-center gap-0.5"
                            title={`Search ${a} in Salesforce`}
                          >
                            {a}
                          </a>
                        ))}
