...
const ACCURACY_CORRECTIONS: AccuracyCorrection[] = [
  ...
  },
  {
    ...
    wrongClaim: 'Direct-to-onboarding (skip the Provider)',
    correction: 'Every person must go through the Provider first. Fastest path: evergreen job → add candidate → move to Hired → Start Onboarding.',
  },
  {
  ...
  },
  {
    ...
    wrongClaim: 'WhatsApp works like SMS in Service Alignment',
...
export function isProductKnowledgeQuery(query: string): boolean {
  const productPatterns = [
    ...
    /\b(does tt|does servicealignment|do we (have|support|offer|do|integrate)|can (we|tt|servicealignment)|is there a?)\b/i,
...
function formatNoMatchResponse(query: string): string {
  return [
    ...
    '',
    '**Where to check:**',
    '- 📄 [TT Support Docs](https://support.servicealignment.com)',
    '- 📘 [API Docs](https://docs.servicealignment.com)',
