...
export default function DynamicAudiencePage() {
  ...
  const handleFindContacts = async () => {
    ...
    // Grab domains from seeds that have 0 contacts in our enriched map
    const pendingDomains = seeds
      .map(s => s.company_domain)
      .filter(d => (contactCounts.get(d) || 0) === 0);

    if (pendingDomains.length === 0) {
      alert("All accounts in this audience already have contacts!");
      return;
    }

    const confirm = window.confirm(`Ready to discover contacts for ${pendingDomains.length} accounts using Apollo?`);
    if (!confirm) return;

    setIsDiscovering(true);

    try {
      // Send to backend in batches of 10
      let totalFound = 0;
      for (let i = 0; i < pendingDomains.length; i += 10) {
        const batch = pendingDomains.slice(i, i + 10);
        const res = await fetch('/api/discovery/apollo-search', {
          method: 'POST',
        ...
        if (res.ok) {
          ...
          totalFound += result.inserted || 0;
        } else {
          console.error("Batch failed", await res.text());
        }
      }

      alert(`Discovery complete! Inserted ${totalFound} new contacts.`);
      await loadData();
    } catch (err) {
      console.error(err);
            ...
            {isAdmin && seeds.length > 0 && (
              <>
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 text-white text-[11px] font-semibold rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50"
                >
