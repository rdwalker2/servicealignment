...
const TABS: TabDef[] = [
  { id: 'status',       label: 'System Status',   icon: Activity,    roles: ['admin', 'exec'] },
  { id: 'team',         label: 'Team & Access',    icon: Users,       roles: ['admin', 'exec'] },
  { id: 'integrations', label: 'Integrations',     icon: Link2,       roles: ['admin', 'exec'] },
  { id: 'rules',        label: 'Signal Rules',     icon: Sliders,     roles: ['admin', 'exec'] },
  { id: 'ai',           label: 'AI Config',        icon: Sparkles,    roles: ['admin', 'exec'] },
  { id: 'audit',        label: 'Audit Trail',      icon: ScrollText,  roles: ['admin', 'exec'] },
  { id: 'docs',         label: 'How It Works',     icon: HelpCircle,  roles: ['admin', 'exec'] },
];
  ...
  return (
    <div className="space-y-4">
      ...
      <SectionCard title="Security Posture">
        ...
        <StatusRow icon={Users} label="Access Control" status="ok" detail={`${Object.keys(ALLOWED_USERS).length} authorized users · @teamtailor.com only`} />
      </SectionCard>
    </div>
  );
}
...
function TeamAccessTab() {
  ...
  const users = useMemo(() => {
    return Object.entries(ALLOWED_USERS).map(([email, profile]) => ({
      email,
      ...profile,
      context: EXEC_CONTEXT[profile.id] || (profile.role === 'admin' ? 'Platform Administrator' : 'Account Executive'),
      salesloftId: REP_SALESLOFT_MAPPING[profile.id]?.id || null,
    }));
  }, []);

  const roleOrder = { admin: 0, exec: 1, manager: 2, rep: 3 };
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-100">
              ...
              <th className="text-left px-4 py-2.5 text-[10px] text-gray-400 font-bold">Context</th>
              <th className="text-left px-4 py-2.5 text-[10px] text-gray-400 font-bold">Salesloft ID</th>
          ...
          <tbody>
            {sorted.map((u) => (
              <tr key={u.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                ...
                <td className="px-4 py-2.5 text-xs text-gray-600">{u.context}</td>
                <td className="px-4 py-2.5">
                  {u.salesloftId ? (
                    <span className="font-mono text-xs text-gray-600">#{u.salesloftId}</span>
                  ) : (
        ...
        <div className="px-4 py-3 border-t border-gray-100">
          ...
          <div className="space-y-1.5">
            {Object.entries(REP_SALESLOFT_MAPPING).map(([repId, sl]) => {
              const profile = Object.values(ALLOWED_USERS).find(u => u.id === repId);
              return (
      ...
      <SectionCard title="Signal Taxonomy (From Daniel's Framework)">
        <div className="p-4 space-y-3">
          ...
          <div className="text-[10px] text-gray-500 space-y-1.5">
            <div><span className="inline-block w-2 h-2 rounded-full bg-red-500 mr-1.5" />  <strong>Weight 10:</strong> Legacy ATS, Recruiter Overload, Broken Application UX, Manual Workflows, RFP/Vendor Review</div>
      ...
      <SectionCard title="Default Saved Segments">
        <div className="divide-y divide-gray-50">
          {[
            ...
            { name: 'Legacy ATS Vulnerability', desc: 'current_ats contains "Frontline" AND open_roles > 20' },
...
{`You are an elite, top-performing enterprise Account Executive at Teamtailor (a modern, highly aesthetic ATS / Applicant Tracking System with a built-in career site builder).
...
function AuditTrailTab() {
  ...
  const userNameMap = useMemo(() => {
    ...
    Object.entries(ALLOWED_USERS).forEach(([email, profile]) => {
...
function GMDoc() {
  return (
    <div className="space-y-3">
      ...
      <div className="space-y-2 text-xs text-gray-700">
        <div>The <strong>Teamtailor GTM Workspace</strong> is an internal tool that helps the North America sales team find and reach out to the right prospects at the right time.</div>
        ...
        <div><strong>Who has access:</strong> Only authorized @teamtailor.com emails can log in. Authentication is passwordless (magic link). All actions are logged in an audit trail.</div>
    ...
    <div className="space-y-3">
      ...
      <div className="space-y-3">
        {[
          ...
          { q: 'User can\'t log in', a: 'Their email must be in the authorized users list (Team & Access tab). Check for typos. Verify they\'re using @teamtailor.com. In dev mode, login is instant. In production, check spam folder for magic link email.' },
