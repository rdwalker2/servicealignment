...
export function useRealtimeSync() {
  ...
  useEffect(() => {
    ...
    async function init() {
      // 1. Auto-migrate existing local data to new tables if they are empty
      await autoMigrateToTables();

      // 2. Pull all data from tables and hydrate local storage & Zustand
      ...
      // 3. Set up Realtime subscriptions to listen to other users' changes
      setupSubscriptions();
    }
...
async function pullFromTables() {
  ...
  if (sessions && sessions.length > 0) {
    ...
    // Also add local-only sessions (e.g. freshly seeded) that aren't in Supabase yet
    for (const local of localSessions) {
      if (!sessions.find(row => row.data.id === local.id)) {
        arr.push(local);
      }
    }
