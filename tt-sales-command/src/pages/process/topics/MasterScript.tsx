...
import { DocHeader, Callout, ScriptBlock } from '../../../components/process/DocComponents';
import { ChevronDown, ChevronRight, Ear, ChevronsUpDown } from 'lucide-react';

// ─── Collapsible Stage ────────────────────────────────────────────

function Stage({ id, stage, title, subtitle, accent, open, onToggle, children }: {
  id: string; stage: string; title: string; subtitle: string;
  ...
  return (
    <div className="mb-12" id={id}>
      <button
        onClick={onToggle}
        className="w-full flex items-center justify-between text-left group pb-3 border-b border-stone-200"
      >
        <div className="flex items-center gap-2.5">
          ...
          />
          <div>
            <span className="text-[10px] font-semibold uppercase tracking-widest text-stone-400">{stage}</span>
            <div className="flex items-center gap-2">
              <span className="text-[17px] font-bold text-stone-900">{title}</span>
              <span className="text-[12px] text-stone-400 italic hidden md:inline">— {subtitle}</span>
            </div>
          </div>
        </div>
        {open ? <ChevronDown size={14} className="text-stone-300" /> : <ChevronRight size={14} className="text-stone-300" />}
      </button>
      {open && <div className="pt-6 space-y-0">{children}</div>}
    </div>
  );
}
...
function Step({ number, title, isFirst, children }: { number: string; title: string; isFirst?: boolean; children: React.ReactNode }) {
  return (
    <div className={`relative ${isFirst ? '' : 'border-t border-stone-100 pt-8 mt-8'}`}>
      <div className="flex items-baseline gap-2.5 mb-3">
        <span className="text-[11px] font-mono font-bold text-stone-300 shrink-0">{number}</span>
        <p className="font-semibold text-stone-800 text-[14px]">{title}</p>
      </div>
  ...
  return (
    <div className="mt-3 rounded-lg bg-stone-50 px-4 py-3">
      <div className="flex items-center gap-1.5 mb-2">
        <Ear size={11} className="text-stone-400" />
        <span className="text-[10px] font-semibold text-stone-400 uppercase tracking-widest">Listen For</span>
      </div>
      <div className="space-y-1">
        {items.map((item, i) => <p key={i} className="text-[12px] text-stone-500 leading-relaxed">• {item}</p>)}
      </div>
    </div>
  );
}
...
function Pillar({ number, name, children }: { number: string; name: string; accent?: string; children: React.ReactNode }) {
  return (
    <div className="pt-2">
      <p className="text-[10px] font-semibold uppercase tracking-widest text-stone-400 mb-0.5">Pillar {number}</p>
      <p className="text-[14px] font-bold text-stone-800 mb-3">{name}</p>
      {children}
    </div>
  );
}
...
export default function MasterScript() {
  ...
  const [openStages, setOpenStages] = useState<Record<string, boolean>>({
    opener: true, d1: true, d2: true, d3: true, d4: true,
  });
  ...
  const toggleAll = useCallback(() => {
    ...
    setOpenStages({ opener: newVal, d1: newVal, d2: newVal, d3: newVal, d4: newVal });
  }, [allOpen]);

  const jumpTo = useCallback((id: string) => {
    // Expand the target stage if collapsed
    setOpenStages(prev => ({ ...prev, [id]: true }));
    // Scroll to it
    setTimeout(() => {
  ...
  return (
    <div ref={scrollRef} className="p-10 max-w-5xl mx-auto h-full overflow-y-auto pb-32 relative">
      <DocHeader
        stage="MASTER"
        title="The Complete Teamtailor Script"
        subtitle="End-to-end discovery-to-close. Zero placeholders. Every word specific to what we sell and who we sell to."
      />

      {/* ── Sticky jump nav ── */}
      <div className="sticky top-0 z-20 -mx-10 px-10 py-2.5 bg-white/90 backdrop-blur-sm border-b border-stone-100 mb-8 flex items-center justify-between">
        <div className="flex items-center gap-1">
          {STAGES.map(s => {
            const accent = ACCENT[s.id as keyof typeof ACCENT] || ACCENT.pre;
            return (
              <button
                key={s.id}
                onClick={() => jumpTo(s.id)}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-md text-[11px] font-semibold text-stone-500 hover:text-stone-800 hover:bg-stone-50 transition-colors"
              >
        ...
        <button
          onClick={toggleAll}
          className="flex items-center gap-1 px-2.5 py-1.5 rounded-md text-[10px] font-semibold text-stone-400 hover:text-stone-600 hover:bg-stone-50 transition-colors uppercase tracking-wider"
        >
      ...
      <div className="mb-10">
        <p className="text-[14px] leading-relaxed text-stone-500">
          ...
          Every talk track is written for Teamtailor, with real data points, real customer references, and real
          language that maps to our prospects' actual pain.
        </p>
      </div>

      {/* ═══════ PRE-CALL: THE OPENER ═══════ */}
      <Stage id="opener" stage="Pre-Call" title="The Opener" subtitle="Set the frame before anything else" accent={ACCENT.pre} open={openStages.opener} onToggle={() => toggle('opener')}>
        <Step number="1" title="The Frame — First 60 Seconds" isFirst>
...
I'd rather be honest with you than waste your time. Fair?"`}
      ...
      {/* ═══════ D1: URGENCY TEST ═══════ */}
      <Stage id="d1" stage="D1" title="Urgency Test" subtitle="Do they need to act?" accent={ACCENT.d1} open={openStages.d1} onToggle={() => toggle('d1')}>

        <Step number="Q1" title="What's the Strategic Initiative?" isFirst>
...
steps is that, and where does it get stuck?"`}
          ...
          <ListenFor items={[
            '"We\'re drowning" / "I\'m at my wits end" — manual process pain (70% of deals)',
            '"Our system sucks" / "It\'s an afterthought" — HRIS displacement play (40%)',
            '"We don\'t have anything" — first-ATS, fastest close cycle',
            '"We\'re growing fast" / "We can\'t keep up" — scaling trigger',
          ]} />
        </Step>

        <Step number="Q1c" title="Map the Process (Feeds CMS: jk1-jk6)">
...
about how hiring works today — what would it be?"`}
          ...
          <ListenFor items={[
            'HM feedback via email/Slack = zero audit trail → TT scorecard + mobile app',
            '"I haven\'t checked mobile" → pull up their career site live, visual contrast with TT',
            '"We re-enter data" → TT onboarding module + HRIS webhook (ADP, Paylocity, etc.)',
            'Magic wand answer = centerpiece of your demo',
          ]} />
        ...
        <Step number="Q1b" title="How Critical Is This?">
...
[If 1-3:] "Sounds like the timing might not be right. What
        ...
        <Step number="Q2" title="Who Else Is Involved?">
...
way we're not playing telephone."`}
          ...
          <ListenFor items={[
            'Solo decision maker = fastest close (Pedro: signed same day)',
            '"I need to check with..." = identify the real buyer early',
            '"Our HR Director / CFO / CEO would need to approve" = map the chain',
            'Show rate is the #1 team-wide problem — get decision makers in early',
          ]} />
        </Step>

        <Step number="Q3" title="What Infrastructure Exists Today?">
...
hiring process end-to-end? ATS, job boards, scheduling,
...
data need to flow into it?"`}
          ...
          <ListenFor items={[
            'HRIS name = trigger the afterthought wedge ("recruiting is a checkbox")',
            '"We have 10-15 systems" — fragmentation pain (El Cortez)',
            '"Paper applications" — massive modernization opportunity',
            '"It does the job but..." — they\'re settling',
            '4+ systems = strong consolidation case (cs1 → CMS)',
            '2+ locations = TT Divisions feature (js1 → CMS)',
            'HRIS name = integration play (js2 → CMS: ADP, Paylocity, Workday, BambooHR)',
          ]} />
        </Step>

        <Step number="Q4" title="The Cost of Inaction">
...
another 90 days?"`}
        ...
        <Step number="Q4b" title="ROI Quantification (Feeds Calculator)">
...
[Note → cs2. If <10% or "we don't track this" = TT AI screening pitch.]`}
        ...
        <Step number="✓" title="D1 Confirmation Close">
...
is just how. Let me dig into that."`}
      ...
      {/* ═══════ D2: GAP TEST ═══════ */}
      <Stage id="d2" stage="D2" title="Gap Test" subtitle="Do they need outside help?" accent={ACCENT.d2} open={openStages.d2} onToggle={() => toggle('d2')}>

        <Step number="Q5" title="What Have You Tried Before?" isFirst>
          <ScriptBlock label="Surface Past Failures + Competitive Landscape">
{`"Have you evaluated or invested in a solution like this
before?

[If yes — they've had an ATS:]
"What happened? What worked and what didn't?"

[If switching from HRIS ATS module:]
...
(Radical honesty builds trust.)`}
          ...
          <ListenFor items={[
            'Paycom/ADP/Workday = win rates 78-91% — these are our best displacement targets',
            'Lever/Greenhouse = harder fight (55-72%) — need sharper differentiation',
            '"We tried but it didn\'t work" — find out WHY and position TT as the fix',
            '"Contract renews in [month]" — natural switching window, create timeline urgency',
            'Competitor name → activates Kill Page in CMS (cs3 → competitor_situation)',
          ]} />
        </Step>

        <Step number="Q6" title="Root Cause Diagnosis + Problem Reframe">
...
ATS is clunky' or 'screening takes too long'].
...
Does that resonate?"`}
          ...
          <Callout type="info" title="Why the Reframe Matters">
            If you agree with their diagnosis ("yeah, your ATS is clunky"), any vendor can win. When you reframe to the BIGGER problem (capacity, candidate experience, data blindness), you position Teamtailor as the only solution that addresses the root cause.
          </Callout>
        </Step>

        <Step number="Q6b" title="The Most Important Question in the BAP">
...
show you how we solve this."`}
        ...
        <Step number="Q7" title="How Do They Buy?">
...
time."`}
        ...
        <Step number="Q8" title="Budget & Timeline">
          <ScriptBlock label="Budget Discovery — Teamtailor">
...
Does that timeline work for you?"`}
        ...
        <Step number="Q8b" title="Success Vision + Self-Assessment">
...
why we should keep talking."`}
        ...
        <Step number="✓" title="D2 Confirmation Close">
...
we're in a position to help."`}
      ...
      {/* ═══════ D3: SOLUTION ═══════ */}
      <Stage id="d3" stage="D3" title="Solution Presentation" subtitle="Are we the best solution?" accent={ACCENT.d3} open={openStages.d3} onToggle={() => toggle('d3')}>

        <Step number="1" title="Proven Results — Establish Credentials (3 min)" isFirst>
...
What questions do you have before I show you the platform?"`}
        ...
        <Step number="2" title="Clear Solution — The Treatment Plan (12 min)">
...
Teamtailor works across three pillars:"`}
          </ScriptBlock>

          <div className="mt-4 space-y-6">
            <Pillar number="1" name="Attract" accent={ACCENT.d3}>
...
[Show a Teamtailor customer career site]
...
free. No manual posting."`}
            ...
            <Pillar number="2" name="Automate" accent={ACCENT.d1}>
...
the cracks."`}
            ...
            <Pillar number="3" name="Optimize" accent="#f59e0b">
...
Ask after each pillar. Identify where they want depth.`}
        ...
        <Step number="3" title="Timeline & Implementation (3 min)">
...
first 3 weeks. Fair enough?"`}
        ...
        <Step number="3b" title="Champion Enablement + Success Metrics">
...
→ HM satisfaction → Mobile app + adoption metrics`}
        ...
        <Step number="4" title="Price Presentation (2 min)">
...
move forward. Now present price.`}
          ...
          <ScriptBlock label="Presenting Price — Teamtailor">
...
Great — let me walk you through the last part."`}
      ...
      {/* ═══════ D4: DECISION ═══════ */}
      <Stage id="d4" stage="D4" title="Confirmation Close" subtitle="No maybes allowed" accent={ACCENT.d4} open={openStages.d4} onToggle={() => toggle('d4')}>

        <Step number="Q12" title="Trial Close — Confidence Check" isFirst>
...
Teamtailor is the right solution for [company]?"
...
now than waste your time."`}
        ...
        <Step number="1" title="The Alignment Checklist">
...
✅ Teamtailor solves this through [reference the 1-2
...
Should I send it over?"`}
        ...
        <Step number="3" title="Handling Final Resistance">
...
ATS is bolted onto payroll, it's not built for
recruiting'].
...
looked at that and you're still confident, I respect it."`}
        ...
        <Step number="✓" title="Post-Call: Lock the Next Step">
