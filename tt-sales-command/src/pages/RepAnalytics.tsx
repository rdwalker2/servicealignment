...
export default function RepAnalytics() {
  const { user, isManager, effectiveUser } = useAuth();
  ...
  // All sessions for the selected rep
  ...
  const repName = repProfile?.full_name ?? effectiveUser?.full_name ?? 'Rep';
        ...
        >
          <div>
            ...
            <p className="text-sm text-stone-500 ml-10.5">
              {repName} · <span className="text-stone-700 font-medium">{distribution.total}</span> deals analyzed
            </p>
          </div>
