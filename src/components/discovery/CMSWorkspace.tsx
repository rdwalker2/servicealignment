...
import { ROOFING_PAINS as TT_PAINS } from '../../data/painData';
...
import { AutoBridgeBanner } from './AutoBridgeBanner';
...
interface CMSWorkspaceProps {
  ...
  flashSave: () => void;
  activeTab: 'bap' | 'meddpicc' | 'settings' | 'analytics' | 'call';
  onTabChange: (tab: 'bap' | 'meddpicc' | 'settings' | 'analytics' | 'call') => void;
  predictiveData?: any;
}
...
export function CMSWorkspace({
  ...
  activeTab, onTabChange, predictiveData
}: CMSWorkspaceProps) {
  // ── Modal state ──
  ...
  const competitiveConfig = () => {
    ...
    const currentAts = session.membrane_type;
    const currentAtsLabel = MEMBRANE_OPTIONS.find(a => a.id.toLowerCase() === currentAts?.toLowerCase())?.name ?? currentAts;
    const competitorChoices = MEMBRANE_OPTIONS.filter(a =>
      a.id !== 'none' && a.id !== 'other' && a.id.toLowerCase() !== currentAts?.toLowerCase()
    );
    ...
    return (
      <div className="space-y-4">
        ...
        <div>
          <label className="text-[9px] font-bold text-stone-400 mb-1.5 block">Current Membrane System</label>
          {currentAts && currentAts !== 'none' ? (
            ...
            <p className="text-[10px] text-stone-400 italic">No Membrane selected — set in Deal Context above</p>
          )}
        </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5 rounded-lg border border-stone-100 bg-stone-50/50 p-2.5">
            {competitorChoices.map(ats => {
              const isSelected = evaluating.includes(ats.id);
              return (
                <button
                  key={ats.id}
                  onClick={() => toggleEvaluating(ats.id)}
  ...
  return (
    <>
      <div className="flex-1 min-h-0 flex flex-col" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
        ...
        {/* ── Call Companion (full-width, no sidebar) ── */}
        {activeTab === 'call' && (
          <div className="flex-1 min-h-0">
            <CallMode
              ...
              onSwitchToFull={() => onTabChange('bap')}
            />
          </div>
        )}
        ...
        {/* ── CMS Tab (default) ── */}
        {(activeTab === 'bap' || activeTab === 'settings' || activeTab === 'analytics') && (
          ...
          <div className="w-[380px] shrink-0 flex flex-col border-r border-stone-200 bg-[#f8f7f6]">
            ...
            <div className="shrink-0 px-4 py-3 border-b border-stone-200 bg-white flex items-center gap-2">
              <div className="flex-1 flex items-center gap-1.5 min-w-0">
                <span className="text-[11px] font-black text-stone-800 truncate">{companyName || 'New Deal'}</span>
                {(session as any).company_id && (
                  <a
                    href={`https://servicealignment.lightning.force.com/lightning/r/Account/${(session as any).company_id}/view`}
                    target="_blank"
                ...
                {(session as any).sf_opportunity_id && (
                  <a
                    href={`https://servicealignment.lightning.force.com/lightning/r/Opportunity/${(session as any).sf_opportunity_id}/view`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="opacity-40 hover:opacity-100 transition-opacity shrink-0"
              ...
              <span className="text-[9px] text-emerald-500 font-semibold flex items-center gap-1 mr-2">
            ...
            {/* ── Auto-Bridge Banner ── */}
            <AutoBridgeBanner session={session} onSessionChange={handleSessionChange} />
                ...
                <Collapsible title="BAP Context" defaultOpen>
                  <div className="space-y-3">
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div>
                        <label className="text-[9px] font-bold text-stone-400 mb-1 block">Current Membrane</label>
                        <select
                          value={session.membrane_type ?? ''}
                          onChange={e => handleSessionChange({ ...session, membrane_type: e.target.value })}
                          className="w-full rounded-lg border border-stone-200 bg-stone-50 px-3 py-1.5 text-[11px] text-stone-700 focus:outline-none focus:border-stone-400 focus:bg-white transition-colors"
                        >
                          <option value="">Select...</option>
                          {MEMBRANE_OPTIONS.map(ats => <option key={ats.id} value={ats.id}>{ats.name}</option>)}
                        </select>
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="text-[9px] font-bold text-stone-400 mb-1 block">Square Footage</label>
                        <input
                          type="number"
                          min={1}
                          value={session.roof_square_footage || ''}
                          onChange={e => handleSessionChange({ ...session, roof_square_footage: Number(e.target.value) })}
                          placeholder="50,000"
                          className="w-full rounded-lg border border-stone-200 bg-stone-50 px-3 py-1.5 text-[11px] text-stone-700 placeholder:text-stone-300 focus:outline-none focus:border-stone-400 focus:bg-white transition-colors"
                        />
                      </div>
                      <div>
                        <label className="text-[9px] font-bold text-stone-400 mb-1 block">Building Use Case</label>
                        <select
                          value={session.building_use_case ?? ''}
                          onChange={e => handleSessionChange({ ...session, building_use_case: e.target.value })}
                          className="w-full rounded-lg border border-stone-200 bg-stone-50 px-3 py-1.5 text-[11px] text-stone-700 focus:outline-none focus:border-stone-400 focus:bg-white transition-colors"
                        >
                          <option value="">Select...</option>
                          {BUILDING_USE_CASES.map(i => <option key={i} value={i}>{i}</option>)}
                        </select>
                      </div>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      >
                        <option value="">Select...</option>
                        <option value="replacing-ats">Replacing Provider</option>
                        <option value="first-ats">First Provider</option>
                ...
                <div className="relative">
                  ...
                  {showAddSection && (
                    <div className="absolute left-0 right-0 top-full mt-1 bg-white rounded-xl border shadow-lg z-10 py-1 max-h-60 overflow-y-auto">
                      ...
                      {['Executive Summary', 'Challenges', 'Features', 'Why Dedicated Provider', 'Investment', 'Next Steps'].map(label => (
                        <button
                          key={label}
                          onClick={() => {
                            const widgets = (session as any).library_widgets ?? [];
