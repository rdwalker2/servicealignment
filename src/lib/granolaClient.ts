...
const GRANOLA_API_BASE = '/granola-api';
...
export function noteMatchesCompany(note: GranolaApiNote, companyName: string): boolean {
  ...
  ].filter(e => e && !e.includes('servicealignment.com')); // exclude internal
  ...
  for (const a of attendees) {
    if (!a.email?.includes('servicealignment.com') && a.name) {
 ...
 */
export function extractMEDDPICCFromSummary(markdown: string, attendees: any[] = []): Partial<MEDDPICCExtraction> {
  if (!markdown) return {};
  ...
  if (result.next_steps) {
    ...
    if (bullets.length > 0) {
      const INTERNAL_TEAM_EMAILS_DOMAIN = 'servicealignment.com';
      // Fallback: Known internal team names (update this list when team changes)
      const TT_NAMES = ['ryan', 'jack', 'jamie', 'brian', 'chris', 'anna', 'james', 'nic', 'tyler', 'moe'];

      // Extract internal member names based on attendee emails
      const internalAttendees = attendees?.filter(a => a.email?.toLowerCase().includes(INTERNAL_TEAM_EMAILS_DOMAIN)) || [];
      const internalNamesFromDomain = internalAttendees.map(a => a.name?.toLowerCase() || '').filter(Boolean);
      ...
      for (const bullet of bullets) {
        ...
        const isInternalByDomain = internalNamesFromDomain.some(n => n.includes(owner.toLowerCase()) || owner.toLowerCase().includes(n));
        const isInternalByFallback = TT_NAMES.some(n => owner.toLowerCase().includes(n));
        const isInternal = isInternalByDomain || isInternalByFallback;
...
// ── Native Markdown Extraction ──

export interface NativeExtractedNextSteps {
  who: string;
  what: string;
  when: string;
}

export function extractNextStepsFromMarkdown(markdown: string): NativeExtractedNextSteps {
  const result: NativeExtractedNextSteps = { who: '', what: '', when: '' };
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.match(/^#+\s*(Salesforce\s*)?Next\s*Steps/i)) {
      inNextSteps = true;
      continue;
    }
    
    // Stop if we hit a new section
    if (inNextSteps && line.startsWith('#')) {
      break;
    }

    if (inNextSteps) {
      // Strip bullet points like "- ", "* ", or "1. " from the start of the line, and strip bolding
      const cleanLine = line.replace(/^[-*]\s+/, '').replace(/^\d+\.\s+/, '').replace(/\*\*/g, '').trim();

      if (cleanLine.toLowerCase().startsWith('who:')) {
        result.who = cleanLine.replace(/^who:/i, '').trim();
      } else if (cleanLine.toLowerCase().startsWith('what:')) {
        result.what = cleanLine.replace(/^what:/i, '').trim();
      } else if (cleanLine.toLowerCase().startsWith('when:')) {
        result.when = cleanLine.replace(/^when:/i, '').trim();
      }
    }
  }
...
export async function fetchMEDDPICCForCompany(companyName: string): Promise<{
  ...
  try {
    ...
    const extraction = extractMEDDPICCFromSummary(details.summary_markdown || '', details.attendees);
