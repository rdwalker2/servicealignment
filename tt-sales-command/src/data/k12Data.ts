...
export const NO_ATS_VALUES = new Set([
  'Unknown', 'Not Scanned', 'No ATS Detected', '',
]);

export type AtsCategory = 'none' | 'paper' | 'jobboard' | 'legacy' | 'modern' | 'teamtailor' | 'unknown';

export function classifyAts(ats: string): AtsCategory {
  if (ats === 'Teamtailor') return 'teamtailor';
...
export function computePriority(d: K12District): number {
  ...
  // ATS scoring — Paper/Manual is the HOTTEST lead
  ...
  // Apptegy user bonus (modern brand, no ATS)
...
export function generateSalesBrief(d: K12District): string {
  ...
  const atsVal = (d.current_ats && d.current_ats !== 'Not Scanned' && d.current_ats !== '') ? d.current_ats : 'No ATS';
