...
import { getPredictiveDataByToken, type PredictiveEngineData } from '../../lib/predictiveEngineDb';
    ...
    <div className="flex min-h-screen items-center justify-center bg-zinc-50" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      >
        ...
        <p className="mb-6 text-sm leading-relaxed text-zinc-500">
          We couldn't find a business case for "{slug}". Please contact your Service Alignment representative for an updated link.
        </p>
      </motion.div>
    </div>
  );
    ...
    <div className="flex min-h-screen items-center justify-center bg-stone-950 px-4">
      <div className="text-center">
        <div className="mb-6 flex justify-center">
          <img src="/Color-Logo-SA-Web.webp" alt="Service Alignment" className="h-16 w-auto object-contain" />
        </div>
...
export default function PublicBusinessCase() {
  ...
  // Use a pseudo-session for legacy props down to BusinessCaseDocument
  const [session, setSession] = useState<any | null>(null);
  const [predictiveData, setPredictiveData] = useState<PredictiveEngineData | null>(null);
  ...
  useEffect(() => {
    async function loadData() {
      ...
      const data = await getPredictiveDataByToken(token);
      if (data) {
        setPredictiveData(data);
        // Create a pseudo-session for the BusinessCaseDocument component compatibility
        const mainProperty = data.properties.length > 0 ? data.properties[0].property : null;
        setSession({
          company_name: mainProperty ? mainProperty.property_name : 'Valued Client',
          cms_prepared_by: 'Service Alignment Predictive Engine',
          cms_accent: '#C8932E',
          call_sheet_answers: {
            q1: mainProperty ? `Your property at ${mainProperty.address} has triggered a health alert.` : 'We have detected signals across your portfolio.',
          },
          room_visibility: {
            ...DEFAULT_VISIBILITY,
            hero: true,
          ...
          roof_health_score: data.properties.length > 0 ? data.properties[0].healthScore?.health_score : 68,
          roof_signals: data.properties.length > 0 ? data.properties[0].signals : []
        });
        setLoadState('ready');
      } else {
        setLoadState('notfound');
  ...
  if (loadState === 'notfound' || !session) return <NotFound slug={token} />;
  ...
  const repName = (session as any).cms_prepared_by || 'Service Alignment';
    ...
    <div className="min-h-screen bg-white" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      >
        <div className="mx-auto flex h-16 max-w-5xl items-center justify-between px-6">
          <div className="flex items-center gap-3">
            <img src="/Color-Logo-SA-Web.webp" alt="Service Alignment" className="h-7 w-auto object-contain brightness-0 invert" />
      ...
      <DocErrorBoundary>
        <BusinessCaseDocument
          ...
          session={session}
          predictiveData={predictiveData} // Pass the new predictive data
          visibility={visibility}
          isRepPreview={false}
          currentApproach={answers['q5'] as string}
          rootCause={answers['q6'] as string}
          biggerProblemOverride={undefined}
          problemStatement={answers['q1'] as string}
          businessImpact={answers['q4a'] as string}
          urgencyReason={answers['q4b'] as string}
          customMessage={`Based on automated monitoring of ${companyName}.`}
          repName={repName}
          showPricing={true}
      ...
      <footer className="border-t border-zinc-100 py-8" style={{ background: `linear-gradient(180deg, white, ${themeColor}05)` }}>
        <div className="mx-auto max-w-6xl px-4 sm:px-6 text-center">
          ...
          <div className="flex items-center justify-center gap-4 text-xs text-zinc-400">
            <span>Powered by <strong className="text-zinc-600">Service Alignment</strong></span>
          </div>
        </div>
      </footer>
    </div>
