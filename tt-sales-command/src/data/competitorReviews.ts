...
// relevant quotes when the prospect selects their ATS + pains
// ============================================================
...
const LEVER_REVIEWS: CompetitorReview[] = [
  ...
  },
  {
    ...
    quote: "Lever was supposed to be the CRM-forward ATS. After 2 years, our talent pools are just lists with no engagement. No nurture campaigns, no re-engagement triggers.",
    source: 'Capterra',
    stars: 3,
...
const BAMBOOHR_REVIEWS: CompetitorReview[] = [
  ...
  },
  {
    ats: 'bamboohr',
    painIds: ['poor-candidate-experience', 'poor-mobile-career'],
    quote: "Candidates tell me the application felt 'cold and impersonal.' No brand, no personality. It's an HR admin tool pretending to be an ATS.",
    source: 'Reddit',
...
const SMARTRECRUITERS_REVIEWS: CompetitorReview[] = [
  ...
  },
  {
    ats: 'smartrecruiters',
    painIds: ['high-cost-per-hire', 'hris-ats-rigidity'],
    quote: "We had Teamtailor very high on the list all the time. The only reason we favored Smart Recruiters was the native SAP integration. But there's quite a big misalignment — and the pricing.",
    source: 'Prospect Call',
...
const PAYCOM_REVIEWS: CompetitorReview[] = [
  {
    ats: 'paycom',
    painIds: ['high-cost-per-hire', 'hris-ats-rigidity'],
    quote: "We're paying double what a standalone ATS costs, and the recruiting module is the worst part of Paycom. It's a payroll tool forcing us to recruit through payroll workflows.",
    source: 'G2',
    stars: 2,
    reviewerRole: 'VP of HR',
    companySize: '201-500 employees',
...
const RIPPLING_REVIEWS: CompetitorReview[] = [
  {
    ...
    quote: "Rippling is brilliant for onboarding and IT provisioning but the ATS is bare bones. No career site builder, no CRM, no nurture campaigns. We outgrew it for recruiting fast.",
    source: 'G2',
    stars: 3,
    reviewerRole: 'Head of People',
    companySize: '51-200 employees',
...
const PAYLOCITY_REVIEWS: CompetitorReview[] = [
  {
    ...
    quote: "Paylocity is what it is. It's just an applicant tracking system. It is not a people attraction system.",
    source: 'Prospect Call',
    reviewerRole: 'HR Director',
    companySize: '201-500 employees',
    date: 'May 2026',
...
/**
 * Get competitor reviews matching a specific ATS and any of the selected pain IDs.
...
export function getRelevantReviews(ats: string | null, selectedPainIds: string[], maxCount: number = 4): CompetitorReview[] {
  ...
  // Always return up to maxCount reviews for the ATS, even if score is 0,
  // to ensure the social proof grid always looks full (min 3).
  return scored.slice(0, maxCount).map(s => s.review);
}

/**
 * Get all reviews for a given ATS (regardless of pain selection).
 */
...
/**
 * Get the display name for an ATS key.
 */
