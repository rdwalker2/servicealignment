...
export function useSignalFilters(
    ...
    const loadSegments = async () => {
      ...
      if (data && data.length > 0) {
        const persisted: Segment[] = data.map(d => ({
          ...
          name: d.name,
          icon: d.icon,
          conditions: Array.isArray(d.conditions) ? (d.conditions as FilterCondition[]) : [],
        }));
  ...
  const filteredRows = useMemo(() => {
    ...
    if (activeSegment && Array.isArray(activeSegment.conditions) && activeSegment.conditions.length > 0) {
  ...
  const contacts = useMemo(() => {
    // Flatten contacts from all merged accounts (includes CRM contacts, not just signaled ones)
    const fromAccounts = accounts.flatMap(a => a.contacts);
    // Also include any signal-only contacts not already covered
    const signalContacts = groupByContact(filteredRows);
    const seen = new Set(fromAccounts.map(c => c.email));
    const extra = signalContacts.filter(c => !seen.has(c.email));
    const all = [...fromAccounts, ...extra]
      .filter(c => c.name && c.name !== 'Unknown' && c.name !== 'Unknown Contact');
    return sortDir === 'asc' ? [...all].reverse() : all;
  }, [accounts, filteredRows, sortDir]);
  ...
  return {
    ...
    segmentCounts: segments.reduce((acc, seg) => {
      ...
      if (conds.length === 0) acc[seg.id] = accounts.length;
      else acc[seg.id] = clayRows.filter(r => evaluateConditions(conds, r)).length;
      return acc;
