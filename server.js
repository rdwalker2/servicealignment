...
// ── 10-min Granola Auto-Sync ──
// Polls Granola API for new meeting notes, matches to deals, runs LLM extraction
cron.schedule('3,13,23,33,43,53 * * * *', async () => {
  try {
    if (!process.env.GRANOLA_API_KEY) return;
    console.log('[Granola Cron] Running auto-sync...');
    const result = await runGranolaSync();
    console.log(`[Granola Cron] Done: ${result.matched} matched, ${result.skipped} skipped`);
  } catch (err) {
    console.error('[Granola Cron] Error:', err.message);
  }
});
...
// ── Granola API Proxy ──
// Proxies /granola-api requests to Granola's public API with server-side auth.
// This fixes the missing proxy that granolaClient.ts expects.
app.use('/granola-api', createProxyMiddleware({
  target: 'https://public-api.granola.ai/v1',
  changeOrigin: true,
  pathRewrite: { '^/granola-api': '' },
  on: {
    proxyReq: (proxyReq) => {
      if (process.env.GRANOLA_API_KEY) {
        proxyReq.setHeader('Authorization', `Bearer ${process.env.GRANOLA_API_KEY}`);
      }
      proxyReq.setHeader('Accept', 'application/json');
    },
  },
...
// ── Granola Sync Health ──
app.get('/api/health/granola', async (req, res) => {
  res.json({
    configured: !!process.env.GRANOLA_API_KEY,
...
app.post('/api/discovery/apollo-search', async (req, res) => {
  try {
    const { domains, titles } = req.body;
    if (!domains || !Array.isArray(domains)) {
      return res.status(400).json({ error: 'domains array is required' });
    }
    const result = await discoverContacts(domains, titles);
    res.status(200).json(result);
  } catch (err) {
...
// ── AI Business Case Co-Pilot ──
import { runBusinessCaseAgent } from './roof-business-case-agent.js';

app.post('/api/agent/business-case', async (req, res) => {
  try {
    const { address } = req.body;
    if (!address) {
      return res.status(400).json({ error: 'address is required' });
    }
    const result = await runBusinessCaseAgent(address);
    res.status(200).json(result);
  } catch (err) {
...
// IMPORTANT: Resolves the correct Salesloft user ID for the rep making the call,
// so the call is attributed to them (not the API key owner) in both SL and Salesforce.

// Maps internal rep IDs → Salesloft User IDs (must stay in sync with rep_roster table)
const REP_TO_SL_USER = {
  'rep-jl': 12457,  // Jack Luther
  'rep-ma': 12427,  // Moe Aqel
  'rep-th': 12418,  // Tyler Hanson
};

app.post('/api/salesloft/log-call', async (req, res) => {
  try {
    const { email, phone, sentiment, disposition, duration, notes, caller_rep_id } = req.body;
    ...
    // SAFETY: Check DNC before logging call
    if (person.do_not_contact) {
      console.warn(`[Call Log] BLOCKED — ${email} is marked Do Not Contact`);
      return res.status(403).json({ error: 'Contact is marked Do Not Contact in Salesloft', email });
    }

    // 2. Resolve the caller's Salesloft user ID
    // Use the rep mapping if a rep ID is provided, otherwise fall back to /v2/me
    let userId;
    if (caller_rep_id && REP_TO_SL_USER[caller_rep_id]) {
      userId = REP_TO_SL_USER[caller_rep_id];
      console.log(`[Call Log] Using rep mapping: ${caller_rep_id} → SL user ${userId}`);
    } else {
      // Fallback: use the API key owner (admin users, or if rep mapping is missing)
      const meResp = await fetch('https://api.salesloft.com/v2/me', { headers: slHeaders });
      const meData = await meResp.json();
      userId = meData.data?.id;
      if (caller_rep_id) {
        console.warn(`[Call Log] No SL mapping for rep ${caller_rep_id}, falling back to /v2/me (user ${userId})`);
      }
    }
    ...
    console.log(`[Call Log] Logged call for ${email} by rep ${caller_rep_id || 'admin'}: ${disposition} (${duration}s)`);
...
app.get('/msi-viking/business-case', (req, res) => {
  res.sendFile(path.join(__dirname, 'dist', 'msi-viking', 'business-case', 'index.html'));
});
app.get('/msi-viking/business-case/', (req, res) => {
  res.sendFile(path.join(__dirname, 'dist', 'msi-viking', 'business-case', 'index.html'));
});
...
// ── WebSocket Server for Live Weather Alerts ──
const wss = new WebSocketServer({ server });
wss.on('connection', (ws) => {
  console.log('[WebSocket] Client connected for live alerts');
  ws.on('close', () => console.log('[WebSocket] Client disconnected'));
});

// Export wss so we can broadcast alerts from weatherMonitor
export { wss };

// Start the Live Radar Skill
import { startWeatherMonitor } from './src/lib/skills/weatherMonitor.ts';
startWeatherMonitor(wss);
