...
export interface RepCoachingProfile {
  ...
  // high = criticalDeals > 2 OR avgBAPScore < 30
  // medium = atRiskDeals > 2 OR avgBAPScore < 60
  // low = otherwise
}
...
function getCoachingPriority(
  ...
  if (criticalDeals > 2 || avgBAPScore < 30) return 'high';
  if (atRiskDeals > 2 || avgBAPScore < 60) return 'medium';
  return 'low';
}
...
export function buildCoachingProfiles(): RepCoachingProfile[] {
  ...
  const profiles: RepCoachingProfile[] = REP_PROFILES.map(rep => {
    ...
    const coachingPriority = getCoachingPriority(criticalDeals, atRiskDeals, avgBAPScore);
