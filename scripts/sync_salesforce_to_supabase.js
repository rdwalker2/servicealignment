...
async function main() {
  ...
  dbSessions.forEach(session => {
    ...
    if (deal.company_name) {
      dbByCompanyName[deal.company_name.toLowerCase().trim()] = session;
    }
  });
  ...
  csvDeals.forEach(csvDeal => {
    ...
    const sfOppId = csvDeal['Opportunity ID'];
    ...
    // Keep qualifying, investigating, evaluating, negotiating, contracting, signing
    // (If the stage from SFDC is exactly these, it maps perfectly).
    ...
    if (!dbSession) {
      ...
      const newData = {
        ...
        company_id: sfAccountId, // We store SF Account ID as company_id
        selected_pains: [],
        roi_inputs: {
      ...
      // 2. Existing Deal -> Update if mismatched
      const dbDeal = dbSession.data;
      const dbStage = dbDeal.deal_stage || '';
      const dbAmount = dbDeal.deal_value || 0;

      if (dbStage.toLowerCase() !== mappedStage || dbAmount !== amount || !dbDeal.sf_opportunity_id || !dbDeal.company_id) {
        const updatedData = {
          ...dbDeal,
          deal_stage: mappedStage,
          deal_value: amount,
  ...
  console.log('✅ Sync complete. All accounts and deals updated with SFDC IDs!');
}
