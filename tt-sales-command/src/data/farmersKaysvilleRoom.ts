...
export function seedFarmersKaysvilleRoom(): string {
  ...
  const session: any = {
    ...
    bap_answers: {
      ...
      q10: 'maybe', // Solution Map: Showed ATS + career site + scheduling — Tim hasn't seen yet
    ...
    bap_notes: {
      q1: "Apex Talent (Farmers' internal ATS/CRM) is being sunset across ALL Farmers locations. They just canceled IdealTraits annual renewal because it wasn't delivering applicants via Indeed/LinkedIn/ZipRecruiter. Currently falling back to KSL (Utah-only job board).",
      ...
      q10: "Showed career site builder (Eden: 'my boss would like that'), ATS pipeline, AI screening, interview scheduling (Google Cal + Teams), messaging (email + SMS). Eden positive on every feature.",
    ...
    competitive_situation: "AgencyZoom (primary): Insurance sales CRM with candidate tracking as secondary feature. 'Free' via shared login. NOT an ATS — cannot post to job boards or build career sites. Shared login risk flagged.\n\nIdealTraits (displaced): Former posting tool. Canceled — wasn't delivering.\n\nApex Talent (sunsetting): Farmers' internal CRM. Going away for ALL locations. Was 'a note-taking location.'",
    ...
    competitors_identified: 'AgencyZoom (free shared login — NOT an ATS, only candidate tracker)',
    competitors_count: 1,
    ...
    objections: [
      ...
      },
      {
        ...
        response: "Explained this is a job board issue (not ATS-specific). Recommended periodic relisting, location variants. TT integrations make refreshes faster.",
