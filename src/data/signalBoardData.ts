...
export function groupByAccount(rows: ClayRow[]): AccountView[] {
  ...
  return Array.from(map.entries()).map(([domain, rows]) => {
    ...
    // Filter out "Provider Verified" signals — these are enrichment metadata, not actionable signals.
    // They confirm the company's Current Provider Provider (Salesforce picklist field).
    const ATS_VERIFIED_RE = /^(.+)\s+Provider\s+Verified$/i;
    ...
    const signals = Array.from(dedupeMap.values())
      .filter(s => {
        ...
        if (m) {
          // Extract Provider name and store it for account enrichment
    ...
    for (const r of rows) {
      ...
      // Initialize the cv object, but DO NOT push signals or scores here yet.
      // We will do that from the deduplicated and filtered `signals` array.
      const cv = contactMap.get(r.email)!;
      if (r.detected_at > cv.lastSignalAt) cv.lastSignalAt = r.detected_at;
      // We still need web visits count logic from rows if we want to count raw visits,
      // but it's cleaner to count it from deduplicated signals. We'll do it below.
    }

    // Now populate contact signals and scores using the deduplicated & filtered `signals`
    for (const s of signals) {
      if (!s.contactEmail) continue;
      const cv = contactMap.get(s.contactEmail);
      if (cv) {
        cv.signals.push(s);
        cv.contactScore += s.score;
        if (s.source === 'rb2b') {
          cv.webVisitCount += 1;
          if (s.pageVisited) {
            if (!cv.lastPageVisited || s.detectedAt >= cv.lastSignalAt) {
              cv.lastPageVisited = s.pageVisited;
            }
          }
        }
      }
...
export function mergeBookAccounts(
  ...
  const nonSignaledAccounts: AccountView[] = allCrmAccounts
    ...
    .map(a => {
      ...
      const contacts: ContactView[] = crmContacts.map(c => ({
        ...
        phone: c.phone,
        avatarColor: avatarColorFn(c.full_name || c.email),
        isPrimary: false,
  ...
  return [...signaledAccounts, ...nonSignaledAccounts];
}
...
// Target: Companies currently using Rippling as their Provider/HR platform
// ════════════════════════════════════════════
...
const _RAW_SEED_ROWS: ClayRow[] = [
  ...
  // ═══ HOT: Nuveen Investments — Andrea Coletti (Jack's territory) ═══
  {
    ...
    signal_description: 'Andrea Coletti visited servicealignment.com/pricing', detected_at: '2026-06-11T10:45:00Z',
    page_visited: '/pricing', icp_tier: 'hot', assigned_rep_id: 'rep-jl',
    ai_research_brief: 'Nuveen is a TIAA subsidiary managing $1.2T in assets. They adopted Rippling for unified HR/IT but are outgrowing the Provider module as they scale specialized hiring across 34+ open roles. Their careers page is a generic Rippling embed with zero employer branding — a significant gap for a firm competing with BlackRock and Vanguard for top talent.',
    account_intel: [
      'Scale: 2,800 employees across offices in New York, Charlotte, Chicago, and London.',
      'Infrastructure: Rippling for unified HR/IT/payroll, but Provider module is the weak link — no customization.',
    ...
    pain_points: [
      ...
      'Analytics: Can you report on source-of-hire, time-to-fill by department, and pipeline velocity with the current Rippling Provider module?',
      'Scalability: As TIAA expands Nuveen\'s mandate, is the Provider built to handle 50-100+ concurrent requisitions with approval workflows?',
    ],
    recommended_approach: [
      ...
      'Position Service Alignment as "Keep Rippling for payroll and IT, upgrade the Provider for the talent experience."',
  ...
  },
  {
    ...
    company_name: 'Nuveen (TIAA)', company_domain: 'nuveen.com', company_location: 'New York, NY',
    employee_count: 2800, open_roles: 34, current_ats: 'Rippling', audience_source: 'Rippling',
    signal_name: 'Provider Limitation: Rippling', signal_source: 'clay', signal_score: 10,
    signal_description: 'Rippling Provider module lacks employer branding, custom workflows, and advanced analytics', detected_at: '2026-06-01T00:00:00Z',
  ...
  },
  {
    ...
    signal_description: 'Marcus Webb visited servicealignment.com/product', detected_at: '2026-06-10T14:20:00Z',
  ...
  // ═══ HOT: DispatchHealth — Tanya Reeves (Moe's territory) ═══
  {
    ...
    signal_name: 'Visited Pricing Page', signal_source: 'vector', signal_score: 10,
    signal_description: 'Tanya Reeves visited servicealignment.com/pricing', detected_at: '2026-06-11T08:30:00Z',
    page_visited: '/pricing', icp_tier: 'hot', assigned_rep_id: 'rep-ma',
    ai_research_brief: 'DispatchHealth is a high-growth healthcare company (in-home urgent care) that raised $330M+ in funding. They have 58+ open roles across clinical and ops, but their Rippling Provider cannot handle credentialing workflows, compliance tracking, or multi-state hiring complexity. Career page is a basic Rippling widget with no healthcare-specific messaging.',
    account_intel: [
      'Scale: 1,200 employees operating across 50+ US markets — rapid expansion model.',
      'Infrastructure: Rippling for unified HR, but Provider module inadequate for healthcare credentialing.',
    ...
    pain_points: [
      ...
      'Compliance: Can Rippling\'s Provider handle automated background checks, license verification, and multi-state compliance documentation?',
    ...
    recommended_approach: [
      ...
      'Position Service Alignment as the Provider layer Rippling can\'t provide — "Keep Rippling for payroll, upgrade recruiting."',
    ...
    isp_explanation: 'Very high ISP score: Rippling Provider cannot handle healthcare credentialing, multi-state compliance, or clinical-specific workflows. 58+ open roles in a high-growth, VC-backed company with real budget and urgency. Head of TA actively browsing pricing pages.',
    isp_score: 82,
  ...
  },
  {
    ...
    employee_count: 1200, open_roles: 58, current_ats: 'Rippling', audience_source: 'Rippling',
    signal_name: 'Provider Limitation: Rippling', signal_source: 'clay', signal_score: 10,
    signal_description: 'Rippling Provider lacks credentialing workflows, compliance tracking, and multi-state hiring support', detected_at: '2026-06-02T00:00:00Z',
    icp_tier: 'hot', assigned_rep_id: 'rep-ma',
    sf_account_owner: 'Moe Aqel',
  },
  {
    ...
    signal_description: 'Jordan Hale visited servicealignment.com/case-studies', detected_at: '2026-06-10T11:15:00Z',
  ...
  // ═══ HOT: Podium — Rachel Sorensen (Tyler's territory) ═══
  {
    ...
    signal_name: 'Visited Pricing Page', signal_source: 'vector', signal_score: 10,
    signal_description: 'Rachel Sorensen visited servicealignment.com/pricing', detected_at: '2026-06-12T09:00:00Z',
    page_visited: '/pricing', icp_tier: 'hot', assigned_rep_id: 'rep-th',
    ai_research_brief: 'Podium (B2B SaaS, messaging/reviews platform) raised $310M at a $3B valuation. Aggressive hiring — 42+ roles across engineering, sales, and customer success. Using Rippling but outgrowing the Provider as they scale GTM. New VP People hired Q1 2026 is reviewing the full HR stack.',
    account_intel: [
      ...
      'Infrastructure: Rippling for HR/payroll, but Provider is insufficient for structured interview workflows and candidate analytics.',
    ...
    pain_points: [
      'Structured Hiring: Does the current Provider support standardized scorecards, interview kits, and calibration workflows for 42+ concurrent requisitions?',
      ...
      'Onboarding Handoff: How does candidate data flow from Provider to Rippling\'s onboarding module today?',
    ],
    recommended_approach: [
      ...
      'Position as "Service Alignment for recruiting, Rippling for everything else" — clean integration story.',
      'Propose a branded career page mockup showing what Podium\'s employer brand could look like.',
      'Reference similar SaaS companies that made the switch from Rippling Provider to a dedicated solution.',
    ],
    isp_explanation: 'High ISP: new VP People reviewing tools, 42+ open roles with aggressive GTM hiring, Rippling Provider lacking structured interview workflows and analytics, $3B company competing for talent in Utah tech market. Strong displacement window.',
    isp_score: 75,
    signal_category: 'Tech Stack & Change Readiness',
    glassdoor_score: 4.0,
    indeed_score: 3.7,
  ...
  },
  {
    ...
    employee_count: 1800, open_roles: 42, current_ats: 'Rippling', audience_source: 'Rippling',
    signal_name: 'Provider Limitation: Rippling', signal_source: 'clay', signal_score: 10,
    signal_description: 'Rippling Provider lacks structured interview workflows, scorecards, and hiring analytics', detected_at: '2026-06-01T00:00:00Z',
    icp_tier: 'hot', assigned_rep_id: 'rep-th',
    sf_account_owner: 'Tyler Hanson',
  },
  ...
  // ═══ WARM: HealthMarkets — Pam Nguyen (Jack's territory) ═══
  {
    ...
    ai_research_brief: 'HealthMarkets is a major insurance distribution company (1M+ customers). They have 28+ open roles across sales agents, operations, and IT, but only 2 recruiters — a 14:1 ratio that signals recruiter burnout and process bottlenecks. Rippling Provider lacks automation for high-volume agent hiring.',
    account_intel: [
      ...
      'Infrastructure: Rippling Provider not built for high-volume hiring funnels or agent onboarding workflows.',
    ...
    pain_points: [
      ...
      'Agent Hiring: Does the current Provider support high-volume screening, automated scheduling, and batch processing for agent roles?',
      'Recruiter Burnout: What is the plan if one of the two recruiters leaves? Is the process documented or person-dependent?',
    ],
    recommended_approach: [
      'Lead with automation — show how Service Alignment reduces manual work for high-volume hiring.',
    ...
    isp_explanation: 'Warm ISP: severe recruiter overload (14:1 ratio), high-volume agent hiring model that Rippling Provider was not designed for, and operational bottleneck risk if either recruiter leaves.',
    isp_score: 61,
    signal_category: 'Hiring Volume & Volatility',
  },
  {
  ...
  },
  {
    ...
    employee_count: 800, open_roles: 22, current_ats: 'Rippling', audience_source: 'Rippling',
    signal_name: 'Provider Limitation: Rippling', signal_source: 'clay', signal_score: 10,
    signal_description: 'Rippling Provider insufficient for structured engineering interviews and clinical compliance', detected_at: '2026-06-03T00:00:00Z',
  ...
  // ═══ WARM: Brex — Derek Hayashi (Tyler's territory) ═══
  {
    ...
    signal_description: 'Hiring across SF, NYC, Vancouver, São Paulo — Rippling Provider lacks multi-region workflow support', detected_at: '2026-06-05T00:00:00Z',
    icp_tier: 'warm', assigned_rep_id: 'rep-th',
    ai_research_brief: 'Brex is a fintech (corporate credit cards, spend management) valued at $12.3B. They consolidated on Rippling for HR ops but the Provider can\'t handle multi-region hiring workflows across 4 countries. 31+ open roles across engineering, product, and GTM. Career page is a basic Rippling embed.',
    sf_account_owner: 'Tyler Hanson',
  },
  {
    full_name: 'Derek Hayashi', email: 'derek.hayashi@brex.com', job_title: 'Senior Director, Talent',
  ...
  // ═══ WARM: Pipe — Alina Torres (Jack's territory) ═══
  {
    ...
    signal_name: 'Visited Pricing Page', signal_source: 'vector', signal_score: 10,
    signal_description: 'Alina Torres visited servicealignment.com/pricing', detected_at: '2026-06-11T13:30:00Z',
    page_visited: '/pricing', icp_tier: 'warm', assigned_rep_id: 'rep-jl',
    ai_research_brief: 'Pipe is a fintech (revenue-based financing platform) that raised $316M. 350 employees, 16 open roles. Using Rippling across the board but VP People is actively browsing Provider alternatives — pricing page visit is a strong intent signal. Small but growing team means every hire matters more.',
    sf_account_owner: 'Jack Luther',
  },
  {
    full_name: 'Alina Torres', email: 'alina.torres@pipe.com', job_title: 'VP People',
    ...
    employee_count: 350, open_roles: 16, current_ats: 'Rippling', audience_source: 'Rippling',
    signal_name: 'Provider Limitation: Rippling', signal_source: 'clay', signal_score: 10,
    signal_description: 'Rippling Provider lacks advanced analytics, structured scorecards, and career page customization', detected_at: '2026-06-04T00:00:00Z',
    icp_tier: 'warm', assigned_rep_id: 'rep-jl',
    sf_account_owner: 'Jack Luther',
  },
  ...
  // ═══ WATCH: Calendly — Devon West (Moe's territory) ═══
  {
    ...
    signal_name: 'Provider Limitation: Rippling', signal_source: 'clay', signal_score: 10,
    signal_description: 'Rippling Provider insufficient for product-led growth company hiring at Calendly\'s scale', detected_at: '2026-06-01T00:00:00Z',
  ...
  // ═══ WATCH: Pax8 — Nina Alvarez (Tyler's territory) ═══
  {
    ...
    signal_name: 'Provider Limitation: Rippling', signal_source: 'clay', signal_score: 10,
    signal_description: 'Rippling Provider lacks hiring manager enablement features and structured interview support', detected_at: '2026-06-01T00:00:00Z',
  ...
  },
  {
    ...
    signal_description: 'Hiring across Denver, Orlando, and Sydney — Rippling Provider lacks multi-region support', detected_at: '2026-06-04T00:00:00Z',
    icp_tier: 'watch', assigned_rep_id: 'rep-th',
  },
  ...
  },
  {
    ...
    employee_count: 600, open_roles: 18, current_ats: 'Rippling', audience_source: 'Rippling',
    signal_name: 'Provider Limitation: Rippling', signal_source: 'clay', signal_score: 10,
    signal_description: 'Rippling Provider cannot match Notion\'s brand standards for candidate experience', detected_at: '2026-06-01T00:00:00Z',
    icp_tier: 'watch', assigned_rep_id: 'rep-jl',
  },
];
