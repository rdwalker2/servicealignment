...
const SINGLE_MEETING_RECIPE_TEXT = `Analyze this meeting and extract the following in this exact format:
...
const MULTI_MEETING_RECIPE_TEXT = `Analyze these meetings across the entire deal and extract the following in this exact format:

Q1 — Their Objective: [Yes/Maybe/No] — [answer from across all calls]
Q2 — Decision Makers: [Yes/Maybe/No] — [aggregated from all calls]
Q3 — Expected Outcomes/ROI: [Yes/Maybe/No] — [napkin math from relevant call]
Q4 — Priority & Urgency: [Yes/Maybe/No] — [worst case scenario discussed]
Q5 — Other Providers: [Yes/Maybe/No] — [current Provider, what's not working]
Q6 — Problem Diagnosis: [Yes/Maybe/No] — [bigger problem identified]
Q7 — Buying Process: [Yes/Maybe/No] — [approval steps, reactive vs proactive]
Q8 — Budget & Timelines: [Yes/Maybe/No] — [budget range, go-live date]
Q9 — Proven Results: [Yes/Maybe/No] — [what case studies resonated]
Q10 — Clear Solution: [Yes/Maybe/No] — [what demo'd, reaction]
Q11 — Delivery & Timelines: [Yes/Maybe/No] — [implementation discussed]
Q12 — Trial Close: [Yes/Maybe/No]

M — Metrics: [measurable outcomes discussed]
E — Economic Buyer: [name, title, access level]
I — Identify Pain: [core pains in their words]
D — Decision Criteria: [what they care about]
D — Decision Process: [approval chain]
P — Paper Process: [procurement steps]
C — Champion: [internal advocate, strength]
C — Competition: [other Provider evaluated]
...
export function GranolaSettingsDrawer({ isOpen, onClose }: GranolaSettingsDrawerProps) {
  ...
  const [recipeType, setRecipeType] = useState<'single' | 'multi'>('single');
  ...
  const handleCopyRecipe = async () => {
    const textToCopy = recipeType === 'single' ? SINGLE_MEETING_RECIPE_TEXT : MULTI_MEETING_RECIPE_TEXT;
    try {
 ...
 autoExtract ? 'translate-x-[18px]' : 'translate-x-[3px]'
          ...
          <div className="bg-stone-50 border border-stone-200 rounded-xl p-4 space-y-3">
            ...
            <div className="flex items-center gap-2">
              <select
                value={recipeType}
                onChange={(e) => setRecipeType(e.target.value as 'single' | 'multi')}
                className="px-3 py-1.5 text-xs font-medium text-stone-600 bg-stone-100 hover:bg-stone-200 rounded-lg border-none focus:ring-0 cursor-pointer"
              >
                <option value="single">Single Meeting</option>
                <option value="multi">Multiple Meetings</option>
              </select>
              <button
            ...
            {recipeExpanded && (
              <div className="relative">
                ...
                <pre className="bg-stone-900 text-stone-300 font-mono text-xs p-4 rounded-xl overflow-x-auto max-h-[320px] overflow-y-auto whitespace-pre-wrap leading-relaxed">
                  {recipeType === 'single' ? SINGLE_MEETING_RECIPE_TEXT : MULTI_MEETING_RECIPE_TEXT}
                </pre>
              </div>
            )}
          </div>
