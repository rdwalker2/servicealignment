...
const PILLARS: AuditPillar[] = [
  {
    ...
    levels: [
      ...
      { score: 3, label: 'Functional', description: 'Provider exists but lots of manual steps remain.' },
  ...
  },
  {
    ...
    realPain: '"Our current provider sucks." — 40% of deals involve replacing an HRIS vendor\'s Provider module that was never built for recruiting. The afterthought problem.',
    discoveryQuestions: [
    ...
    levels: [
      { score: 1, label: 'Nothing', description: 'No Provider at all. Email and spreadsheets.' },
      { score: 2, label: 'Afterthought', description: 'HRIS with basic Provider module. "It came with it."' },
      { score: 3, label: 'Underused', description: 'Dedicated Provider but using less than half the features.' },
      { score: 4, label: 'Capable', description: 'Using most features, a few pain points.' },
      { score: 5, label: 'Optimized', description: 'Fully utilizing a modern, dedicated Provider.' },
    ],
  },
  {
    id: 'intelligence',
  ...
  },
  {
    ...
    listenFor: [
      ...
      '"I spend most of my time on admin" — doing the Provider\'s job',
      'Interest in AI but no idea what\'s available',
    ],
    wave3Link: 'Wave 3 #9 (AI Compliance) or #10 (Provider Utilization)',
    levels: [
...
function getScoreInterpretation(avg: number): { label: string; color: string; close: string } {
  if (avg <= 2.0) return {
    ...
    color: 'text-rose-700',
    close: `"Based on what we just walked through, you scored a ${avg.toFixed(1)} out of 5. The biggest gaps are in [lowest pillars]. This is exactly the type of situation we built Service Alignment for. Want to see what it looks like when these are solved? Takes 20 minutes."`,
  };
  if (avg <= 3.0) return {
      ...
      <DocSection title="The Concept">
        <div className="bg-stone-900 text-white p-6 rounded-xl">
          ...
          <p className="text-[15px] leading-relaxed mt-3">
            They get real value whether they buy or not. You get a structured discovery that naturally exposes every problem Service Alignment solves. <strong>The audit IS the Three Waves framework, delivered as a service.</strong>
          </p>
        </div>
      </DocSection>
...
of companies have significant gaps in their Provider and only 3% use it
