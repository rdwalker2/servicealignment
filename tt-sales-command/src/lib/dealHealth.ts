...
export function computeDealHealth(session: DiscoverySession): DealHealth {
  ...
  if (answers.q10 !== 'yes') {
    allNudges.push({
      severity: 'info',
      message: 'Solution map incomplete — confirm: Is it completely clear how Teamtailor fixes their specific problem?',
    });
  }
  ...
  if (answers.q6 === null && (answers.q5 !== null || answers.q7 !== null)) {
    ...
    allNudges.push({
      severity: 'warning',
      message: '🔺 Price risk: The bigger problem hasn\'t been diagnosed. Without problem elevation, they\'ll see your price as too high for a simple ATS. Complete root cause diagnosis before discussing cost.',
    });
  }
