...
export async function fetchCustomers(options: {
  ...
  // ── Previous ATS filter (supports special '__has_ats__' value) ──
...
export async function fetchCustomerStats(): Promise<CustomerStats> {
  ...
  const [
    ...
    supabase.from('customers').select('*', { count: 'exact', head: true }).eq('segment', 'Enterprise'),
    supabase.from('customers').select('*', { count: 'exact', head: true }).eq('segment', 'Medium'),
    supabase.from('customers').select('*', { count: 'exact', head: true }).eq('segment', 'Small'),
    supabase.from('customers').select('billing_country'),
    supabase.from('customers').select('industry').not('industry', 'is', null),
    supabase.from('customers').select('*', { count: 'exact', head: true }).not('previous_ats', 'is', null),
    supabase.from('customers').select('*', { count: 'exact', head: true }).eq('phase', 'Success'),
    supabase.from('customers').select('*', { count: 'exact', head: true }).not('nps_score', 'is', null),
    supabase.from('customers').select('*', { count: 'exact', head: true }).eq('nps_classification', 'Promoter'),
    supabase.from('customers').select('*', { count: 'exact', head: true }).eq('nps_classification', 'Passive'),
    supabase.from('customers').select('*', { count: 'exact', head: true }).eq('nps_classification', 'Detractor'),
  ]);
...
export async function fetchFilterOptions(): Promise<FilterOptions> {
  const [
    ...
    supabase.from('customers').select('region'),
    supabase.from('customers').select('industry').not('industry', 'is', null),
    supabase.from('customers').select('previous_ats').not('previous_ats', 'is', null),
    supabase.from('customers').select('billing_country'),
    supabase.from('customers').select('phase'),
    supabase.from('customers').select('segment'),
    supabase.from('customers').select('employee_range'),
  ]);
