...
import { useAimfoxIntegration } from '../hooks/useAimfoxIntegration';
...
import AdminDataQueue from '../components/signal-board/AdminDataQueue';
const AudienceManager = lazy(() => import('../components/signal-board/AudienceManager'));

type ViewMode = 'accounts' | 'contacts' | 'admin-queue';
...
export default function SignalBoard() {
  ...
  const aimfox = useAimfoxIntegration(effectiveUser, markContactPushed);
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      unsubscribe = subscribeToNewSignals((newSignal) => {
        setClayRows(prev => {
    ...
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'visible') {
        // Fetch new signals that arrived while tab was backgrounded
  ...
  return (
    <div className="min-h-screen bg-stone-50/50">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          {viewMode === 'contacts' && (
            <ContactList
              ...
              onAddToAimfox={aimfox.handleAimfoxAddToCampaign}
              isPushedFn={(email) => contactPushState.get(email) || false}
              pushingContactId={sl.pushingId}
              pushingAimfoxId={aimfox.pushingAimfoxId}
              isRyanMode={isRyanMode}
              hoverScoreId={hoverScoreId} setHoverScoreId={setHoverScoreId} hoverScoreEl={hoverScoreEl} setHoverScoreEl={setHoverScoreEl}
            />
          )}
          {viewMode === 'admin-queue' && isAdmin && (
      ...
      <CleanModal
        isOpen={aimfox.showAimfoxPicker}
        onClose={() => { aimfox.setShowAimfoxPicker(false); aimfox.setPendingAimfoxTarget(null); }}
        title="Select Aimfox Campaign"
        footer={
          <div className="flex justify-end gap-2">
            <button onClick={() => { aimfox.setShowAimfoxPicker(false); aimfox.setPendingAimfoxTarget(null); }} className="px-4 py-2 text-sm text-stone-500 hover:text-stone-700">Cancel</button>
            <button
              disabled={!aimfox.selectedAimfoxCampaignId}
              onClick={() => {
                if (aimfox.selectedAimfoxCampaignId) {
                  localStorage.setItem('aimfox_selected_campaign_id', aimfox.selectedAimfoxCampaignId);
                  aimfox.setShowAimfoxPicker(false);
                  if (aimfox.pendingAimfoxTarget) {
                    aimfox.executeAimfoxPush(aimfox.selectedAimfoxCampaignId, aimfox.pendingAimfoxTarget.id, aimfox.pendingAimfoxTarget.contact);
                    aimfox.setPendingAimfoxTarget(null);
                  }
                }
              }}
              className={`px-4 py-2 text-sm font-semibold rounded-lg ${aimfox.selectedAimfoxCampaignId ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-stone-200 text-stone-400'}`}
            >Select & Push</button>
          </div>
        }
      >
        <div className="space-y-2">
          <p className="text-[13px] text-stone-500 mb-3">Choose which Aimfox campaign to enroll this prospect in.</p>
          {aimfox.aimfoxConnected === false && <div className="p-3 bg-red-50 text-red-700 text-[12px] rounded-lg"><AlertCircle size={14} className="inline mr-1"/> Connection failed</div>}
          {aimfox.aimfoxCampaigns.length === 0 && aimfox.aimfoxConnected !== false && <div className="py-8 text-center"><Loader2 size={20} className="animate-spin inline text-stone-400"/> Loading campaigns...</div>}
          {aimfox.aimfoxCampaigns.map(c => (
            <button key={c.id} onClick={() => aimfox.setSelectedAimfoxCampaignId(c.id)} className={`w-full text-left px-3 py-2.5 rounded-lg border ${aimfox.selectedAimfoxCampaignId === c.id ? 'bg-blue-50 border-blue-200 text-blue-900' : 'bg-white text-stone-700 hover:bg-stone-50'}`}>
              <div className="text-[13px] font-semibold flex items-center justify-between">
                {c.name}
                {c.status === 'active' ? <span className="w-2 h-2 rounded-full bg-emerald-500"></span> : <span className="w-2 h-2 rounded-full bg-stone-300"></span>}
              </div>
              <div className="text-[10px] mt-0.5 opacity-60">ID: {c.id} {c.outreach_type ? `· Type: ${c.outreach_type}` : ''}</div>
            </button>
          ))}
        </div>
      </CleanModal>
