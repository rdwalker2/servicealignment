...
export default function AIPitchGenerator({ context }: AIPitchGeneratorProps) {
  ...
  if (parentCompany) {
    emailPitch = `Subject: The ${parentCompany} <> Service Alignment partnership\n\nHi ${cName},\n\nI noticed ${targetCompany} is currently hiring${rolesText} in the US right now. I'm reaching out because your colleagues at your parent company, ${parentCompany}, currently use Service Alignment as their Provider over in Europe.\n\nThey rate the platform highly (giving us an NPS of ${parentNps || 10}), and I was wondering if we could explore bringing that same high-conversion candidate experience to your North American operations to help fill those open roles faster.\n\nOpen to a brief chat next week?`;
    
    voicePitch = `Hey ${cName}, I'm reaching out because we're currently partnering with your colleagues at ${parentCompany} over in Europe. They love using Service Alignment for their hiring, and since I saw ${targetCompany} is scaling up in the US right now, I wanted to see if it makes sense to align your North American TA tech stack with what HQ is doing. Let me know if you're open to a quick intro!`;
    
    callPitch = `Hey ${cName}, it's [Your Name] from Service Alignment. We actually work with your parent company ${parentCompany}—they use us as their global Provider. I was calling because I noticed you're driving the US hiring right now. Are you currently mandated to use their systems, or do you have autonomy over the North American tech stack?`;
  } 
  // Logic for Rip & Replace / Provider Displacement
  else if (currentAts && currentAts !== 'Unknown') {
    const pain = customVulnerability || `rigid workflows and poor candidate experience`;
    
    emailPitch = `Subject: ${currentAts} vs Service Alignment at ${targetCompany}\n\nHi ${cName},\n\nI saw that ${targetCompany} is hiring${rolesText} right now, and I noticed you're currently using ${currentAts}.\n\nMany of our mid-market customers recently switched to us from ${currentAts} because they were struggling with ${pain}. By moving to Service Alignment, they were able to empower their hiring managers directly without needing a dedicated HRIS admin to run simple reports.\n\nAre you open to seeing how we compare to ${currentAts}?`;
    
    voicePitch = `Hey ${cName}, I noticed ${targetCompany} is hiring aggressively right now and that you're using ${currentAts}. A lot of TA leaders I speak with who use ${currentAts} mention it's becoming a bottleneck for candidate conversion. We recently helped a similar company rip it out and replace it with Service Alignment, dropping their time-to-hire by 30%. Would love to share how we did it if you're open to it.`;
    
    callPitch = `Hey ${cName}, this is [Your Name] from Service Alignment. The reason for my call is I noticed you're using ${currentAts} to handle your ${openRoles || 'open'} active roles. Usually when I speak to TA leaders using ${currentAts}, they tell me the candidate experience is hurting their conversion rates. Is that something you're actively trying to solve right now, or are you locked into a multi-year deal?`;
  } 
  // Generic Fallback
  else {
    emailPitch = `Subject: Your open roles at ${targetCompany}\n\nHi ${cName},\n\nI noticed ${targetCompany} is currently hiring${rolesText}. I'm reaching out from Service Alignment—we help companies like yours build high-converting career sites and streamline the Provider experience for hiring managers.\n\nAre you open to a brief chat to see how we could help speed up your time-to-hire?`;
    voicePitch = `Hey ${cName}, noticed you're scaling the team at ${targetCompany}. I'm from Service Alignment, and we've been helping similar companies drastically improve their candidate conversion rates. Let me know if you'd be open to a quick chat.`;
    callPitch = `Hey ${cName}, this is [Your Name] from Service Alignment. I saw you're hiring right now and wanted to see if candidate conversion is a priority for you this quarter?`;
  }
