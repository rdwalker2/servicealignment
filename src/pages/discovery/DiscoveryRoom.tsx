...
import { PipelineUpdateDrawer } from '../../components/discovery/PipelineUpdateDrawer';
...
import { GranolaSyncModal } from '../../components/discovery/GranolaSyncModal';
...
import { PostCallDebriefModal } from '../../components/discovery/PostCallDebriefModal';
import {
...
import { useSalesloftCalendarSync } from '../../hooks/useSalesloftCalendarSync';
...
const STAGE_CONFIG: Record<string, { label: string; color: string; short: string }> = {
  discovery:     { label: 'Discovery',     color: '#64748b', short: 'D1' },
  diagnosis:     { label: 'Diagnosis',     color: '#0ea5e9', short: 'D2' },
  demo:          { label: 'Demo',          color: '#8b5cf6', short: 'D3' },
  decision:      { label: 'Decision',      color: '#f59e0b', short: 'D4' },
  closed_won:    { label: 'Closed Won',    color: '#10b981', short: 'Won' },
  closed_lost:   { label: 'Closed Lost',   color: '#ef4444', short: 'Lost' },
};
...
function DealCardGrid({
  ...
  // Pipeline stats
  const stageCount = { discovery: 0, diagnosis: 0, demo: 0, decision: 0, closed_won: 0, closed_lost: 0 };
  let totalScore = 0;
  sessions.forEach(s => {
    const key = s.deal_stage ?? 'discovery';
  ...
  const filtered = sessions.filter(s => {
    ...
    if (stageFilter) {
      if ((s.deal_stage ?? 'discovery') !== stageFilter) return false;
    }
    return true;
  });
      ...
      <header className="border-b border-stone-200/60 bg-stone-900">
        <div className="mx-auto flex h-12 items-center justify-between px-6">
          ...
          <div className="flex items-center gap-6">
            <img src="/Color-Logo-SA-Web.webp" alt="Service Alignment" className="h-6 w-auto object-contain brightness-0 invert hidden sm:block" />
      ...
      <div className="border-b border-stone-200/60 bg-white">
        <div className="max-w-6xl mx-auto px-6 py-3">
          <div className="flex items-center gap-4">
            {/* Stage funnel pills */}
            {([
              { key: 'discovery', label: 'D1', color: '#64748b', count: stageCount.discovery },
              { key: 'diagnosis', label: 'D2', color: '#0ea5e9', count: stageCount.diagnosis },
              { key: 'demo', label: 'D3', color: '#8b5cf6', count: stageCount.demo },
              { key: 'decision', label: 'D4', color: '#f59e0b', count: stageCount.decision },
            ] as const).map((s, i) => (
              <button
                key={s.key}
        ...
        {sorted.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
            {sorted.map(s => {
              ...
              const stageKey = s.deal_stage ?? 'discovery';
              const stage = STAGE_CONFIG[stageKey] || STAGE_CONFIG.discovery;
...
export default function DiscoveryRoom({ companyId, repId }: Props) {
  ...
  const [pipelineDrawerConfig, setPipelineDrawerConfig] = useState<{ dateStr?: string; autoTrigger?: boolean } | undefined>();

  // Granola Modal state for Pipeline view
  const [showGranolaModal, setShowGranolaModal] = useState(false);
  const [granolaTargetMeeting, setGranolaTargetMeeting] = useState<string | undefined>();
  ...
  const slcal = useSalesloftCalendarSync();
  
  // Combine Google and Salesloft calendar events
  const combinedCalendarEvents = useMemo(() => {
    return [...gcal.events, ...slcal.events];
  }, [gcal.events, slcal.events]);
  ...
  const stageKey = currentSession?.deal_stage ?? 'discovery';
  const stage = STAGE_CONFIG[stageKey] || STAGE_CONFIG.discovery;

  const [predictiveData, setPredictiveData] = useState<any>(null);

  // Fetch predictive data for the active deal's company to feed the business case
  useEffect(() => {
    if (currentSession?.company_name) {
      import('../../lib/predictiveEngineDb').then(({ getPredictiveDataByCompany }) => {
        getPredictiveDataByCompany(currentSession.company_name).then(data => {
          if (data) setPredictiveData(data);
          else setPredictiveData(null);
        });
      });
    } else {
  ...
  const sessionForCMS = useMemo(() => {
    if (!currentSession) return null;
    if (!predictiveData || predictiveData.properties.length === 0) return currentSession;
    const primaryProp = predictiveData.properties[0];
    return {
      ...currentSession,
      roof_health_score: primaryProp.healthScore?.health_score ?? (currentSession as any).roof_health_score,
      roof_signals: primaryProp.signals ?? [],
    };
  }, [currentSession, predictiveData]);
  ...
  useEffect(() => {
    ...
    if (companyId && loaded.length > 0) {
      const match = loaded.find((s) => s.company_id === companyId);
        ...
        <div className="h-full flex flex-col" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
          ...
          {pipelineView === 'kanban' ? (
            ...
            <PipelineCalendar
              sessions={sessions}
              onSelect={selectSession}
              googleCalendarEvents={combinedCalendarEvents}
              ...
              onOpenTranscriptSync={(session, meetingId) => {
                selectSession(session);
                setGranolaTargetMeeting(meetingId);
                setShowGranolaModal(true);
              }}
              onLogNextSteps={(session, dateStr) => {
                selectSession(session);
                setPipelineDrawerConfig({ dateStr, autoTrigger: true });
              ...
              onLogMeeting={(sessionId, date, type) => {
                ...
                refreshSessions();
                toast(`Meeting logged: ${typeLabel} added to calendar`, 'success');
              }}
              onMarkHeld={(sessionId, milestoneKey, date) => {
                ...
                refreshSessions();
                toast(`Marked as held: ${session.company_name} meeting`, 'success');
              }}
            />
          ) : null}
        </div>
      ...
      <header className="shrink-0 z-50 border-b border-stone-200/60 bg-stone-900">
        <div className="mx-auto flex h-11 items-center justify-between px-4 gap-3">
          ...
          <div className="flex items-center gap-1.5">
            ...
            {/* Pipeline Update button */}
            <button
              onClick={() => setShowPipelineDrawer(true)}
              className="flex items-center gap-1.5 rounded-lg bg-violet-500 hover:bg-violet-600 px-2.5 py-1.5 text-xs font-semibold text-white transition-colors"
              title="Update Pipeline & Next Steps"
            >
              <CalendarDays size={13} />
              <span className="hidden sm:inline">Update Pipeline</span>
            </button>
            ...
            {/* Transcript Sync button */}
            <button
              onClick={() => setShowGranolaSync(true)}
              className="flex items-center gap-1.5 rounded-lg bg-stone-100 hover:bg-stone-200 px-2.5 py-1.5 text-xs font-semibold text-stone-800 transition-colors"
              title="Sync Granola Transcript"
            >
              <Sparkles size={13} className="text-amber-500" />
              <span className="hidden sm:inline">Transcript Sync</span>
            </button>
            {currentSession && (
      ...
      {currentSession && (
        <>
          ...
          <PostCallDebriefModal
            ...
            companyName={currentSession.company_name}
            fieldsExtracted={debriefFieldCount}
            currentNextAction={currentSession.next_action}
            currentNextMeeting={currentSession.next_meeting_date}
            currentGutFeel={currentSession.gut_feel}
            onSave={(data) => {
              const updated = {
                ...currentSession,
