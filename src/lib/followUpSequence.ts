...
const PAIN_BENCHMARKS: Record<string, string> = {
  ...
  'locked-out-managers':
    'When hiring managers are excluded from the Provider workflow, decision cycles stretch by an average of 11 days.',
...
function buildEmail2(
  ...
  const body =
    ...
    `"${benchmark}"\n\n` +
    `${personaLine}` +
    `Service Alignment was built specifically to close this gap — and the results our customers see in the first quarter are meaningful.\n\n` +
    `${cta}\n\n` +
    `Best,\n${repName}`;
...
function buildEmail3(
  ...
  const roiLine = roiFormatted
    ? `Based on the numbers you shared, companies similar to ${companyName} typically realize ${roiFormatted} in reduced agency spend and see measurably faster fill times in their first quarter with Service Alignment.\n\n`
...
export function generateFollowUpSequence(
  ...
  const rep = repName?.trim() || 'Your Service Alignment Rep';
  return [
