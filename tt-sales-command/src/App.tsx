...
import { BookOpen, ChevronDown, Map, TrendingUp, BarChart3, BarChart2, CalendarDays, Database, LogOut, LayoutDashboard, Search, Trophy, Eye, EyeOff, Users, Target, Shield, Phone, FileText, Mail, Clock, PlusCircle, Crosshair, GraduationCap, ChevronsLeft, ChevronsRight, Kanban, Settings, Zap, Utensils, Home, Globe } from 'lucide-react';
...
import PublicBusinessCase from './pages/discovery/PublicBusinessCase';
import SignalBoard from './pages/SignalBoard';
import CustomerUniverse from './pages/CustomerUniverse';
import FarmersUniverse from './pages/FarmersUniverse';
import RipplingAudience from './pages/RipplingAudience';
import K12Universe from './pages/K12Universe';
import HospitalityUniverse from './pages/HospitalityUniverse';
import GlobalExpansionAudience from './pages/GlobalExpansionAudience';
import CompetitorTakeoutAudience from './pages/CompetitorTakeoutAudience';
...
// Version key ensures old fake data is cleared when we switch to real Acquire2Win room
// Version key ensures old fake data is cleared when we switch to real Acquire2Win room
const SEED_VERSION_KEY = 'scc_seed_version';
const CURRENT_SEED_VERSION = 'v19_calendar_force';
...
function ensureSeedData() {
  if (!_seedDataLoaded) {
    ...
    if (currentVersion !== CURRENT_SEED_VERSION) {
      ...
      loadSeedData();
      // Seed deal rooms from SFDC Open Opps
      seedDealRooms();
      // Seed closed-lost deals for the Redemption autopsy experience
      seedClosedLostDeals();
      localStorage.setItem(SEED_VERSION_KEY, CURRENT_SEED_VERSION);
    } else {
      // Always patch milestones on existing sessions (safety net)
      seedDealRooms();
    }
  }
}
  ...
  return (
    ...
    >
      ...
      <div className={`px-3 py-3 flex items-center border-b border-stone-200/60 ${collapsed ? 'justify-center' : 'gap-2'}`}>
        {!collapsed && (
          <>
            <img src="/lg.svg" alt="Teamtailor" className="h-4 w-auto object-contain" />
      ...
      <nav className="flex-1 p-2 space-y-0.5 overflow-y-auto">
        ...
        <SidebarSection label="Prospecting" first={user.role === 'exec'} collapsed={collapsed} />
        <SidebarLink to="/team/signals" icon={TargetSignalIcon} label="Prospecting" collapsed={collapsed} />

        {/* ── AUDIENCES — dynamic from registry ── */}
        <SidebarSection label="Audiences" collapsed={collapsed} />
        {isAdmin && <SidebarLink to="/team/audience/all" icon={Database} label="All Accounts" collapsed={collapsed} />}
...
function CaseStudyWrapper() {
  ...
  const setActiveTab = (tab: string) => {
    const routeMap: Record<string, string> = {
      ...
      battlecards: '/enablement/battlecards',
    };
...
function TAUWrapper() {
  return <Navigate to="/enablement/battlecards" replace />;
}
    ...
    <div className="flex h-screen bg-stone-50 text-stone-900 font-sans selection:bg-[#FF2A7F]/20 selection:text-[#FF2A7F]">
      ...
      <main className="flex-1 overflow-auto">
        ...
        <Routes>
          ...
          <Route path="farmers" element={user.role === 'exec' ? <Navigate to="/team/signals" replace /> : <FarmersUniverse />} />
          <Route path="k12" element={isAdmin ? <K12Universe /> : <Navigate to="/team/signals" replace />} />
          <Route path="hospitality" element={<HospitalityUniverse />} />
          <Route path="global-expansion" element={<GlobalExpansionAudience />} />
          <Route path="competitor-takeout" element={<CompetitorTakeoutAudience />} />
...
export default function App() {
  // Seed realistic discovery sessions exactly once (module-level flag)
  ensureSeedData();
  return (
    <AuthProvider>
      <SyncProvider>
...
function AppRouter() {
  ...
  const isPublicBusinessCase = /^\/[^/]+\/business-case/.test(location.pathname) && !location.pathname.startsWith('/team') && !location.pathname.startsWith('/enablement') && !location.pathname.startsWith('/leadership');
  ...
  // ── Public prospect-facing Discovery Room — no auth, no sidebar ──
  if (isRoomRoute) {
    return (
      <Routes>
        <Route path="/room/:token" element={<ProspectRoom />} />
      </Routes>
    );
  }

  // ── Public business case pages — no auth, no sidebar ──
  if (isPublicBusinessCase || isCmsPreview) {
    return (
      <Routes>
        <Route path="/:slug/business-case" element={<PublicBusinessCase />} />
        <Route path="/cms-preview/:slug/business-case" element={<PublicBusinessCase />} />
      </Routes>
    );
  }
  ...
  return (
    <Routes>
      ...
      <Route path="/enablement/battlecards" element={
      ...
      <Route path="/enablement/rep-workbook" element={
        <LegacyRouteLayout><EnablementHub /></LegacyRouteLayout>
      } />
      ...
      <Route path="/execution/target-accounts" element={<Navigate to="/enablement/battlecards" replace />} />
