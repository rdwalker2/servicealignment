...
import {
  ...
  ExternalLink, Sparkles, MessageSquare, AlertTriangle, Presentation
} from 'lucide-react';
                ...
                {isExpanded && (
                  ...
                  >
                    <div className="space-y-3 px-4 pb-4 pt-1">
                      ...
                      {note.transcript_analysis && (
                        <div className="bg-violet-50/50 rounded-xl border border-violet-100 p-4 space-y-4">
                          <div className="flex items-center gap-2">
                            <Sparkles size={14} className="text-violet-500" />
                            <h4 className="text-xs font-bold uppercase tracking-wider text-violet-700">Call Insights (AI)</h4>
                          </div>
                          
                          <p className="text-sm text-stone-700">{note.transcript_analysis.summary_feedback}</p>

                          {note.transcript_analysis.happy_ears_warnings.length > 0 && (
                            <div className="space-y-2">
                              <h5 className="text-[10px] font-bold uppercase tracking-wider text-rose-600">Happy Ears Warnings</h5>
                              {note.transcript_analysis.happy_ears_warnings.map((warning, i) => (
                                <div key={i} className="flex items-start gap-2 bg-rose-50 text-rose-800 text-xs px-3 py-2 rounded-lg border border-rose-100">
                                  <AlertTriangle size={12} className="shrink-0 mt-0.5 text-rose-500" />
                                  <span>{warning.warning_text}</span>
                                </div>
                              ))}
                            </div>
                          )}

                          {note.transcript_analysis.objections.length > 0 && (
                            <div className="space-y-2">
                              <h5 className="text-[10px] font-bold uppercase tracking-wider text-amber-600">Objection Handling</h5>
                              {note.transcript_analysis.objections.map((obj, i) => (
                                <div key={i} className="bg-white border border-amber-200 rounded-lg p-3 text-xs space-y-2">
                                  <div className="flex justify-between items-start">
                                    <span className="font-bold text-amber-800 uppercase tracking-wider text-[10px] bg-amber-100 px-1.5 py-0.5 rounded">{obj.objection_type}</span>
                                    <span className={`font-bold ${obj.grade === 'Pass' ? 'text-emerald-600' : obj.grade === 'Fail' ? 'text-rose-600' : 'text-amber-600'}`}>{obj.grade}</span>
                                  </div>
                                  <div>
                                    <span className="text-stone-400 font-medium">Prospect:</span>
                                    <p className="text-stone-800 italic mt-0.5">"{obj.prospect_quote}"</p>
                                  </div>
                                  <div>
                                    <span className="text-stone-400 font-medium">Rep:</span>
                                    <p className="text-stone-700 mt-0.5">{obj.rep_response}</p>
                                  </div>
                                  <div className="bg-amber-50 text-amber-900 p-2 rounded mt-2 font-medium">
                          ...
                          <div className="flex items-center gap-4 pt-2 border-t border-violet-100">
                            <div className="flex flex-col">
                              <span className="text-[10px] uppercase font-bold text-violet-400">Talk Ratio</span>
                              <span className="text-sm font-semibold text-violet-700">{Math.round(note.transcript_analysis.talk_ratio * 100)}%</span>
                            </div>
                            <div className="flex flex-col">
                              <span className="text-[10px] uppercase font-bold text-violet-400">Coaching Adherence</span>
                              <span className="text-sm font-semibold text-violet-700">{note.transcript_analysis.coaching_adherence_score}/100</span>
                            </div>
                          </div>
                        </div>
                      )}
