...
function analyzeDealFatality(selectedSymptoms: string[]): FatalityAnalysis | null {
  ...
  if (selectedSymptoms.some(s => SYMPTOMS.trust.includes(s))) {
    return {
      ...
      trainingLink: { label: 'Five Fatalities — Trust', sublabel: 'Rep Workbook · Five Fatalities section', route: '/enablement/rep-workbook' },
    };
  }
  if (selectedSymptoms.some(s => SYMPTOMS.competition.includes(s)) || selectedSymptoms.some(s => SYMPTOMS.price.includes(s))) {
    return {
      ...
      trainingLink: { label: 'Three Checkpoints + HRIS Wedge', sublabel: 'Rep Workbook · Three Checkpoints', route: '/enablement/rep-workbook' },
    };
  }
  ...
  return {
    ...
    trainingLink: { label: 'Rep Workbook — Start Here', sublabel: 'Buyer’s Intent · Doctor vs. Date mindset', route: '/enablement/rep-workbook' },
  };
}
                  ...
                  <div className="p-4 space-y-3">
                    ...
                    {deal.post_mortem_pathway && (() => {
                      ...
                      const routeMap: Record<string, { label: string; route: string }> = {
                        'Trust Fatality': { label: 'Five Fatalities — Rep Workbook', route: '/enablement/rep-workbook' },
                        'Value / Competition Fatality': { label: 'Three Checkpoints — Rep Workbook', route: '/enablement/rep-workbook' },
