...
// intelligent modal that auto-configures based on persona + ATS.
//
...
export function SmartRoomModal({ open, onClose, onCreate, initialValues }: SmartRoomModalProps) {
  ...
  // Reset state on open (use initialValues if provided from Prospecting view)
  useEffect(() => {
    if (open) {
      setCompanyName(initialValues?.companyName ?? '');
      setPersona(null);
      setCurrentATS(initialValues?.currentATS ?? null);
      setIndustry(initialValues?.industry ?? null);
      setCompanySize(initialValues?.companySize ?? null);
      setSelectedPains([]);
      setSuggestedPains([]);
      setShowAdvanced(!!(initialValues?.industry || initialValues?.companySize));
  ...
  const handleCreate = useCallback(() => {
    ...
    onCreate({
      ...
      selectedPains,
      companyDomain: initialValues?.companyDomain ?? null,
    });
    onClose();
  }, [companyName, persona, currentATS, industry, companySize, selectedPains, onCreate, onClose, initialValues]);
