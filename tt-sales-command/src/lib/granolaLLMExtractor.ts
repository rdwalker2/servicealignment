...
function buildSystemPrompt(session: DiscoverySession): string {
  ...
  return `You are an expert sales intelligence analyst trained on the Buyer's Action Plan (BAP) methodology from Sales Autonomy. You extract structured data from meeting transcripts for a B2B SaaS sales team selling Teamtailor (an ATS / employer brand platform).
...
## Deal Intelligence
- contract_end_date: When their current ATS contract ends
...
10. **CRITICAL for selected_pains**: You MUST select 3-7 pain IDs from the EXACT list above. Do NOT invent new IDs. Every ID must exactly match one from the Pain Categories list. If the prospect describes a pain that doesn't perfectly match, pick the CLOSEST existing ID. For example:
    - "Our ATS is shutting down" → use 'vendor-support-gap'
