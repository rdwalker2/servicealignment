...
const PERSONA_REFRAMES: Record<string, BiggerProblem> = {
  ...
  'hiring-manager': {
    ...
    reframeQuestion: "What percentage of your hiring managers actually log into your Provider more than once a month?",
  ...
  'procurement': {
    ...
    reframeQuestion: "When was the last time you benchmarked your Provider vendor's pricing against the market — and did you have a viable migration path?",
...
const PAIN_TO_FEATURE: Record<string, { feature: string; demoArea: string; icon: string }> = {
  ...
  // Capture
  'screening-bottleneck':    { feature: 'AI Co-pilot',               demoArea: 'Capture (Provider & AI)',           icon: '🤖' },
  'manual-screening':        { feature: 'AI Resume Screening',      demoArea: 'Capture (Provider & AI)',           icon: '🤖' },
  'slow-time-to-hire':       { feature: 'Automation & Triggers',    demoArea: 'Capture (Provider & AI)',           icon: '⚡' },
  'recruiter-burnout':       { feature: 'Workflow Automation',      demoArea: 'Capture (Provider & AI)',           icon: '🔥' },
  'locked-out-managers':     { feature: 'Mobile App',                demoArea: 'Capture (Provider & AI)',           icon: '📱' },
  'scorecard-friction':      { feature: 'Simple Scorecards',        demoArea: 'Capture (Provider & AI)',           icon: '⭐' },
  'scheduling-chaos':        { feature: 'Smart Scheduling',         demoArea: 'Capture (Provider & AI)',           icon: '📅' },
  'no-mobile-feedback':      { feature: 'Full Mobile App',          demoArea: 'Capture (Provider & AI)',           icon: '📱' },
  'inconsistent-interviews': { feature: 'Interview Kits',            demoArea: 'Capture (Provider & AI)',           icon: '📋' },
  'compliance-bottlenecks':  { feature: 'GDPR & Compliance',         demoArea: 'Capture (Provider & AI)',           icon: '🛡️' },
  'high-cost-per-hire':      { feature: 'All-in-One Platform',      demoArea: 'Capture (Provider & AI)',           icon: '💰' },
  'dei-reporting-gaps':      { feature: 'DEI Analytics',            demoArea: 'Capture (Provider & AI)',           icon: '📈' },
  'no-pipeline-visibility':  { feature: 'Real-Time Analytics',      demoArea: 'Capture (Provider & AI)',           icon: '📊' },
...
function getDemoFocusAreas(selectedPains: string[]): { icon: string; area: string; features: string[] }[] {
  ...
  const canonicalOrder = ['Captivate (Employer Brand)', 'Capture (Provider & AI)', 'Convert (Talent CRM)'];
          ...
          >
            <div className="mb-5 flex items-center gap-2">
              ...
              <span className="text-xs font-bold uppercase tracking-wider text-zinc-400">
                The Service Alignment Flywheel
              </span>
            </div>
